...
const programs_1 = require("../@codegen/raydium/programs");
...
class RaydiumService {
    ...
    _raydiumProgramId;
    _logger;
    constructor(rpc, raydiumProgramId = programs_1.AMM_V3_PROGRAM_ADDRESS, logger = console) {
        this._rpc = rpc;
    ...
    async getRaydiumPoolLiquidityDistribution(pool, keepOrder = true, lowestTick, highestTick) {
        ...
        const poolState = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_1.fetchMaybePoolState)(this._rpc, pool));
        if (!poolState) {
            throw Error(`Raydium pool state ${pool} does not exist`);
        }
        const poolPrice = lib_1.SqrtPriceMath.sqrtPriceX64ToPrice((0, raydiumBridge_1.toBN)(poolState.sqrtPriceX64), poolState.mintDecimals0, poolState.mintDecimals1);
    ...
    getStrategyWhirlpoolPoolAprApy = async (strategy, pools) => {
        const position = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_1.fetchMaybePersonalPositionState)(this._rpc, strategy.position));
        if (!position) {
            throw Error(`Position ${strategy.position} does not exist`);
        }
        const poolState = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_1.fetchMaybePoolState)(this._rpc, strategy.pool));
        if (!poolState) {
        ...
        const totalApr = new decimal_js_1.default(apr ?? 0).div(100);
        const fee = new decimal_js_1.default(feeApr ?? 0).div(100);
    ...
    getRaydiumPositionAprApy = async (poolPubkey, priceLower, priceUpper, pools) => {
        const poolState = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_1.fetchMaybePoolState)(this._rpc, poolPubkey));
        if (!poolState) {
    ...
    async getRpcClmmPoolInfo(poolId) {
        ...
        const poolState = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_1.fetchMaybePoolState)(this._rpc, (0, kit_1.address)(poolId)));
        if (!poolState) {
            throw Error(`Raydium pool state ${poolId} does not exist`);
        }
        const rpc = lib_1.PoolInfoLayout.decode(Buffer.from(poolAccountInfo.value.data[0], 'base64'));
        const currentPrice = lib_1.SqrtPriceMath.sqrtPriceX64ToPrice((0, raydiumBridge_1.toBN)(poolState.sqrtPriceX64), poolState.mintDecimals0, poolState.mintDecimals1).toNumber();
        const data = {
            ...rpc,
            currentPrice,
