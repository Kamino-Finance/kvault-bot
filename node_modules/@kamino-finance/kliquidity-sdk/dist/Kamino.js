...
exports.Kamino = exports.KSWAP_BASE_API = exports.KAMINO_SCOPE_FEED_ID = exports.HUBBLE_SCOPE_FEED_ID = void 0;
...
const codamaHelpers_1 = require("./utils/codamaHelpers");
...
const raydiumBridge_1 = require("./utils/raydiumBridge");
...
// StrategyConfigOption variants are now accessed as StrategyConfigOption.UpdateCollectFeesFee, etc.
...
// RebalanceType variants are now accessed as RebalanceType.Manual, RebalanceType.PricePercentage, etc.
...
const kSwap_1 = require("./services/kSwap");
...
function appendAccounts(ix, extra) {
...
class Kamino {
    ...
    _raydiumService;
    _meteoraService;
    _kSwapBaseAPI = exports.KSWAP_BASE_API;
    logger;
    /**
     ...
     * @param jupBaseAPI
     */
    constructor(cluster, rpc, globalConfig, programId, whirlpoolProgramId, raydiumProgramId, meteoraProgramId, jupBaseAPI, // unused, kept for backwards compatibility
    kSwapBaseAPI, logger) {
        ...
        this._orcaService = new services_1.OrcaService(rpc, whirlpoolProgramId, this.logger);
        this._raydiumService = new services_1.RaydiumService(rpc, raydiumProgramId, this.logger);
        this._meteoraService = new MeteoraService_1.MeteoraService(rpc, meteoraProgramId, this.logger);
        if (kSwapBaseAPI) {
    ...
    getDisabledTokensPrices = async (collateralInfos) => {
        ...
        const disabledTokensMints = collInfos.filter((x) => x.disabled && x.mint !== pubkeys_1.DEFAULT_PUBLIC_KEY).map((x) => x.mint);
        if (disabledTokensMints.length === 0) {
            return new Map();
        }
        return (0, kSwap_1.getTokensPrices)(this._kSwapBaseAPI, disabledTokensMints, this.logger);
    };
    ...
    getAllStrategiesWithFilters = async (strategyFilters) => {
        const filters = [];
        filters.push({
            dataSize: BigInt((0, accounts_1.getWhirlpoolStrategySize)()),
        });
        filters.push({
            memcmp: {
                offset: 0n,
                bytes: bs58_1.default.encode(accounts_1.WHIRLPOOL_STRATEGY_DISCRIMINATOR),
                encoding: 'base58',
            },
        });
        ...
        return (await this._rpc
            ...
            .send()).map((x) => {
            const res = {
                strategy: (0, accounts_1.getWhirlpoolStrategyDecoder)().decode((0, bytes_1.base64ToBytes)(x.account.data[0])),
                address: x.pubkey,
            };
            return res;
        });
    ...
    getStrategyByKTokenMint = async (kTokenMint) => {
        ...
        const decodedStrategy = (0, accounts_1.getWhirlpoolStrategyDecoder)().decode((0, bytes_1.base64ToBytes)(matchingStrategies[0].account.data[0]));
        return {
    ...
    getStrategyShareData = async (strategy, scopePrices) => {
        ...
        const scopePricesMap = collateralInfos
            ? await this.getScopePricesMap(this.strategyTokenScopeFeedsToArray(this.getAllScopePriceFeedsForStrategy(strategyState.strategy, collateralInfos)), scopePrices)
            : undefined;
     ...
     */
    getAllScopePriceFeedsForStrategy = (strategy, collateralInfos) => {
        const res = {
            tokenAFeed: collateralInfos[Number(strategy.tokenACollateralId)].scopeFeed,
            tokenBFeed: collateralInfos[Number(strategy.tokenBCollateralId)].scopeFeed,
            rewardsFeeds: [undefined, undefined, undefined],
            kaminoRewardsFeeds: [undefined, undefined, undefined],
        };
        if (strategy.reward0Vault !== pubkeys_1.DEFAULT_PUBLIC_KEY && strategy.reward0Vault !== strategy.baseVaultAuthority) {
            res.rewardsFeeds[0] = collateralInfos[Number(strategy.reward0CollateralId)].scopeFeed;
        }
        if (strategy.reward1Vault !== pubkeys_1.DEFAULT_PUBLIC_KEY && strategy.reward1Vault !== strategy.baseVaultAuthority) {
            res.rewardsFeeds[1] = collateralInfos[Number(strategy.reward1CollateralId)].scopeFeed;
        }
        if (strategy.reward2Vault !== pubkeys_1.DEFAULT_PUBLIC_KEY && strategy.reward2Vault !== strategy.baseVaultAuthority) {
            res.rewardsFeeds[2] = collateralInfos[Number(strategy.reward2CollateralId)].scopeFeed;
        }
        for (let i = 0; i < 3; i++) {
            const rewardInfo = strategy.kaminoRewards[i];
            if (rewardInfo.rewardMint !== pubkeys_1.DEFAULT_PUBLIC_KEY && Number(rewardInfo.decimals) > 0) {
                res.kaminoRewardsFeeds[i] = collateralInfos[Number(rewardInfo.rewardCollateralId)].scopeFeed;
            }
        }
        return res;
    };
    strategyTokenScopeFeedsToArray = (feeds) => {
        const res = [feeds.tokenAFeed, feeds.tokenBFeed];
        for (let i = 0; i < 3; i++) {
            if (feeds.rewardsFeeds[i]) {
                res.push(feeds.rewardsFeeds[i]);
            }
        }
        for (let i = 0; i < 3; i++) {
            if (feeds.kaminoRewardsFeeds[i]) {
                res.push(feeds.kaminoRewardsFeeds[i]);
            }
        }
        return res;
    };
    isOraclePricesAccount = (scopePrices) => {
        if (scopePrices instanceof OraclePrices_1.OraclePrices) {
            return true;
        }
        if (!scopePrices || typeof scopePrices !== 'object' || Array.isArray(scopePrices)) {
            return false;
        }
        const maybeOraclePrices = scopePrices;
        return (typeof maybeOraclePrices.oracleMappings === 'string' &&
            (0, kit_1.isAddress)(maybeOraclePrices.oracleMappings) &&
            Array.isArray(maybeOraclePrices.prices));
    };
    getUniqueScopeFeeds = (feeds) => {
        return Array.from(new Set(feeds.filter((feed) => feed !== pubkeys_1.DEFAULT_PUBLIC_KEY)));
    };
    oraclePricesEntriesToMap = (oraclePrices) => {
        return oraclePrices.reduce((map, [feed, prices]) => {
            map[feed] = prices;
            return map;
        }, {});
    };
    getScopePricesMap = async (feeds, scopePrices) => {
        ...
        if (this.isOraclePricesAccount(scopePrices)) {
            return uniqueFeeds.reduce((map, feed) => {
     ...
     */
    legacyGetStrategiesShareData = async (strategyFilters, stratsWithAddresses, collateralInfos, disabledTokensPrices) => {
        const result = [];
        ...
        const fetchBalances = [];
        const collInfos = collateralInfos ? collateralInfos : await this.getCollateralInfos();
        const allScopePrices = strategiesWithAddresses.flatMap((x) => this.strategyTokenScopeFeedsToArray(this.getAllScopePriceFeedsForStrategy(x.strategy, collInfos)));
        ...
        const raydiumStrategies = strategiesWithAddresses.filter((x) => Number(x.strategy.strategyDex) === (0, utils_1.dexToNumber)('RAYDIUM') && x.strategy.position !== pubkeys_1.DEFAULT_PUBLIC_KEY);
        ...
        const orcaStrategies = strategiesWithAddresses.filter((x) => Number(x.strategy.strategyDex) === (0, utils_1.dexToNumber)('ORCA') && x.strategy.position !== pubkeys_1.DEFAULT_PUBLIC_KEY);
        ...
        const meteoraStrategies = strategiesWithAddresses.filter((x) => Number(x.strategy.strategyDex) === (0, utils_1.dexToNumber)('METEORA') && x.strategy.position !== pubkeys_1.DEFAULT_PUBLIC_KEY);
    ...
    getStrategiesShareData = async (strategyFilters, stratsWithAddresses, collateralInfos, disabledTokensPrices) => {
        ...
        const activeStrategies = strategiesWithAddresses.filter((x) => x.strategy.position !== pubkeys_1.DEFAULT_PUBLIC_KEY);
        const poolsAndPositions = await (0, utils_1.batchFetch)([...activeStrategies.map((x) => x.strategy.pool), ...activeStrategies.map((x) => x.strategy.position)], (chunk) => (0, kit_1.fetchEncodedAccounts)(this._rpc, chunk));
        ...
        const raydiumStrategies = [];
        const orcaStrategies = [];
        const meteoraStrategies = [];
        const raydiumPools = new Map();
        const orcaPools = new Map();
        const meteoraPools = new Map();
        ...
        for (let i = 0; i < activeStrategies.length; i++) {
            ...
            switch (Number(strategy.strategy.strategyDex)) {
                case (0, utils_1.dexToNumber)('RAYDIUM'):
                    raydiumPools.set(strategy.strategy.pool, (0, accounts_3.getPoolStateDecoder)().decode(new Uint8Array(poolBuffer)));
                    raydiumPositions.push((0, accounts_3.getPersonalPositionStateDecoder)().decode(new Uint8Array(positionBuffer)));
                    raydiumStrategies.push(strategy);
                    break;
                case (0, utils_1.dexToNumber)('ORCA'):
                    orcaPools.set(strategy.strategy.pool, (0, accounts_2.getWhirlpoolDecoder)().decode(new Uint8Array(poolBuffer)));
                    orcaPositions.push((0, accounts_2.getPositionDecoder)().decode(new Uint8Array(positionBuffer)));
                    orcaStrategies.push(strategy);
                    break;
                case (0, utils_1.dexToNumber)('METEORA'):
                    meteoraPools.set(strategy.strategy.pool, (0, accounts_4.getLbPairDecoder)().decode(new Uint8Array(poolBuffer)));
                    meteoraPositions.push((0, accounts_4.getPositionV2Decoder)().decode(new Uint8Array(positionBuffer)));
    ...
    getBalance = (strategies, pools, positions, fetchBalance, collateralInfos, prices, disabledTokensPrices) => {
        ...
        for (let i = 0; i < strategies.length; i++) {
            ...
            fetchBalances.push(fetchBalance(strategy, pool, position, collateralInfos, prices, disabledTokensPrices).then((balance) => {
                return { balance, strategyWithAddress: { strategy, address } };
            }));
        }
        return fetchBalances;
    };
    getRaydiumBalances = async (strategy, pool, position, collateralInfos, pricesMap, disabledTokensPrices) => {
        ...
        const poolPrice = lib_1.SqrtPriceMath.sqrtPriceX64ToPrice((0, raydiumBridge_1.toBN)(pool.sqrtPriceX64), decimalsA, decimalsB);
        ...
        const dex = (0, utils_1.numberToDex)(Number(strategy.strategyDex));
        if (rebalanceKind === types_1.RebalanceType.PricePercentageWithReset) {
        ...
        else if (rebalanceKind === types_1.RebalanceType.Expander) {
    ...
    collateralInfos, pricesMap, disabledTokensPrices) => {
    ...
    getRaydiumTokensBalances = (strategy, pool, position, mode = 'WITHDRAW') => {
        ...
        const { amountA, amountB } = lib_1.LiquidityMath.getAmountsFromLiquidity((0, raydiumBridge_1.toBN)(pool.sqrtPriceX64), lowerSqrtPriceX64, upperSqrtPriceX64, (0, raydiumBridge_1.toBN)(position.liquidity), mode === 'DEPOSIT');
    ...
    getMeteoraTokensBalances = async (strategy, pool, position) => {
    ...
    getOrcaBalances = async (strategy, pool, position, collateralInfos, pricesMap, disabledTokensPrices, mode = 'WITHDRAW') => {
        const strategyPrices = await this.getStrategyPrices(strategy, collateralInfos, pricesMap, disabledTokensPrices);
        const rebalanceKind = (0, utils_1.numberToRebalanceType)(strategy.rebalanceType);
    ...
    getStrategyTokensBalances = async (strategy, mode = 'WITHDRAW') => {
        if (Number(strategy.strategyDex) === (0, utils_1.dexToNumber)('ORCA')) {
            ...
            const whirlpool = (0, accounts_2.getWhirlpoolDecoder)().decode((0, bytes_1.base64ToBytes)(whirlpoolAcc.data[0]));
            const position = (0, accounts_2.getPositionDecoder)().decode((0, bytes_1.base64ToBytes)(positionAcc.data[0]));
            ...
            const poolState = (0, accounts_3.getPoolStateDecoder)().decode((0, bytes_1.base64ToBytes)(poolStateAcc.data[0]));
            const position = (0, accounts_3.getPersonalPositionStateDecoder)().decode((0, bytes_1.base64ToBytes)(positionAcc.data[0]));
    ...
    getAccountOwner = async (pk) => {
        const acc = await this.getConnection().getAccountInfo(pk, { encoding: 'base64+zstd' }).send();
        if (acc.value === null) {
            throw Error(`Could not fetch mint ${pk}`);
        }
        return acc.value.owner;
    };
    getStrategyBalancesOrca = async (strategy, collateralInfos, scopePricesMap, disabledTokensPrices) => {
        ...
        const whirlpool = (0, accounts_2.getWhirlpoolDecoder)().decode((0, bytes_1.base64ToBytes)(whirlpoolAcc.data[0]));
        const position = (0, accounts_2.getPositionDecoder)().decode((0, bytes_1.base64ToBytes)(positionAcc.data[0]));
        return this.getOrcaBalances(strategy, whirlpool, position, collateralInfos, scopePricesMap, disabledTokensPrices, undefined);
    };
    getStrategyBalancesRaydium = async (strategy, collateralInfos, scopePricesMap, disabledTokensPrices) => {
        ...
        const poolState = (0, accounts_3.getPoolStateDecoder)().decode(new Uint8Array(new Uint8Array(poolStateAcc.data)));
        const position = (0, accounts_3.getPersonalPositionStateDecoder)().decode(new Uint8Array(new Uint8Array(positionAcc.data)));
        return this.getRaydiumBalances(strategy, poolState, position, collateralInfos, scopePricesMap, disabledTokensPrices);
    };
    getStrategyBalancesMeteora = async (strategy, collateralInfos, scopePricesMap, disabledTokensPrices) => {
        const res = await this.getConnection().getMultipleAccounts([strategy.pool, strategy.position]).send();
        ...
        const poolState = (0, accounts_4.getLbPairDecoder)().decode((0, bytes_1.base64ToBytes)(poolStateAcc.data[0]));
        try {
            const position = (0, accounts_4.getPositionV2Decoder)().decode((0, bytes_1.base64ToBytes)(positionAcc.data[0]));
            return this.getMeteoraBalances(strategy, poolState, position, collateralInfos, scopePricesMap, disabledTokensPrices);
        }
        catch (e) {
            return this.getMeteoraBalances(strategy, poolState, undefined, collateralInfos, scopePricesMap, disabledTokensPrices);
        }
    };
    ...
    getAllPrices = async (oraclePrices, collateralInfos, disabledTokensPrices) => {
        ...
        const allOraclePrices = await this.getScopePricesMap(hubbleCollateralInfos.map((collateralInfo) => collateralInfo.scopeFeed), oraclePrices);
        for (const collateralInfo of hubbleCollateralInfos) {
            if (collateralInfo.scopePriceChain &&
                ...
                if (!collateralOraclePrices) {
                    this.logger.warn(`Could not find scope prices for feed ${collateralInfo.scopeFeed.toString()} and mint ${collateralInfo.mint.toString()}`);
        ...
        try {
            const tokensPrices = disabledTokensPrices
                ...
                : disabledTokens.length > 0
                    ? await (0, kSwap_1.getTokensPrices)(this._kSwapBaseAPI, disabledTokens, this.logger)
                    : new Map();
    ...
    getStrategyPrices = async (strategy, collateralInfos, scopePrices, disabledTokensPrices) => {
        ...
        const pricesMap = await this.getScopePricesMap(this.strategyTokenScopeFeedsToArray(this.getAllScopePriceFeedsForStrategy(strategy, collateralInfos)), scopePrices);
        let jupPrices;
        ...
        const tokenAScopeFeed = collateralInfos[Number(strategy.tokenACollateralId)].scopeFeed;
        const aPrice = scope_sdk_1.Scope.isScopeChainValid(tokenA.scopePriceChain)
            ...
            : fallbackTokenAPrice;
        const tokenBScopeFeed = collateralInfos[Number(strategy.tokenBCollateralId)].scopeFeed;
        ...
        const reward0ScopeFeed = collateralInfos[Number(strategy.reward0CollateralId)].scopeFeed;
        ...
        const reward1ScopeFeed = collateralInfos[Number(strategy.reward1CollateralId)].scopeFeed;
        ...
        const reward2ScopeFeed = collateralInfos[Number(strategy.reward2CollateralId)].scopeFeed;
    ...
    getStrategyRange = async (strategy) => {
        ...
        if (Number(stratWithAddress.strategy.strategyDex) === (0, utils_1.dexToNumber)('ORCA')) {
            return this.getStrategyRangeOrca(strategy);
        }
        else if (Number(stratWithAddress.strategy.strategyDex) === (0, utils_1.dexToNumber)('RAYDIUM')) {
            return this.getStrategyRangeRaydium(strategy);
        }
        else if (Number(stratWithAddress.strategy.strategyDex) === (0, utils_1.dexToNumber)('METEORA')) {
    ...
    getStrategyRangeOrca = async (strategy) => {
        ...
        if (strategyState.position === pubkeys_1.DEFAULT_PUBLIC_KEY) {
            ...
            return this.getPositionRangeOrca(strategyState.position, Number(strategyState.tokenAMintDecimals), Number(strategyState.tokenBMintDecimals));
        }
    };
    getStrategyRangeRaydium = async (strategy) => {
        ...
        if (strategyState.position === pubkeys_1.DEFAULT_PUBLIC_KEY) {
            ...
            return this.getPositionRangeRaydium(strategyState.position, Number(strategyState.tokenAMintDecimals), Number(strategyState.tokenBMintDecimals));
        }
    };
    getStrategyRangeMeteora = async (strategy) => {
        ...
        if (strategyState.position === pubkeys_1.DEFAULT_PUBLIC_KEY) {
            ...
            return this.getPositionRangeMeteora(strategyState.position, Number(strategyState.tokenAMintDecimals), Number(strategyState.tokenBMintDecimals));
        }
    };
    ...
    getPositionRangeOrca = async (positionPk, decimalsA, decimalsB) => {
        ...
        const position = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_2.fetchMaybePosition)(this._rpc, positionPk));
        if (!position) {
            return { lowerPrice: utils_1.ZERO, upperPrice: utils_1.ZERO };
        }
        const lowerPrice = new decimal_js_1.default((0, whirlpools_core_1.tickIndexToPrice)(position.tickLowerIndex, decimalsA, decimalsB));
    ...
    getPositionRangeRaydium = async (positionPk, decimalsA, decimalsB) => {
        ...
        const position = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_3.fetchMaybePersonalPositionState)(this._rpc, positionPk));
        if (!position) {
            return { lowerPrice: utils_1.ZERO, upperPrice: utils_1.ZERO };
        }
        const lowerPrice = lib_1.SqrtPriceMath.sqrtPriceX64ToPrice(lib_1.SqrtPriceMath.getSqrtPriceX64FromTick(position.tickLowerIndex), decimalsA, decimalsB);
    ...
    getPositionRangeMeteora = async (positionPk, decimalsA, decimalsB) => {
        ...
        const position = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_4.fetchMaybePositionV2)(this._rpc, positionPk));
        if (!position) {
            return { lowerPrice: utils_1.ZERO, upperPrice: utils_1.ZERO };
        }
        const pool = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_4.fetchMaybeLbPair)(this._rpc, position.lbPair));
        if (!pool) {
            return { lowerPrice: utils_1.ZERO, upperPrice: utils_1.ZERO };
        }
    ...
    getWhirlpools = async (whirlpools) => {
        ...
        const fetched = await (0, utils_1.batchFetch)(uniqueWhirlpools, async (chunk) => (0, codamaHelpers_1.unwrapAccounts)(await (0, accounts_2.fetchAllMaybeWhirlpool)(this._rpc, chunk)));
    ...
    getOrcaPositions = async (positions) => {
        ...
        const fetched = await (0, utils_1.batchFetch)(nonDefaults, async (chunk) => (0, codamaHelpers_1.unwrapAccounts)(await (0, accounts_2.fetchAllMaybePosition)(this._rpc, chunk)));
    ...
    getRaydiumPositions = async (positions) => {
        const nonDefaults = positions.filter((value) => value !== pubkeys_1.DEFAULT_PUBLIC_KEY);
        const fetched = await (0, utils_1.batchFetch)(nonDefaults, async (chunk) => (0, codamaHelpers_1.unwrapAccounts)(await (0, accounts_3.fetchAllMaybePersonalPositionState)(this._rpc, chunk)));
        const fetchedMap = fetched.reduce((map, position, i) => {
            map[nonDefaults[i]] = position;
            return map;
        }, {});
    ...
    getMeteoraPositions = async (positions) => {
        const nonDefaults = positions.filter((value) => value !== pubkeys_1.DEFAULT_PUBLIC_KEY);
        const fetched = await (0, utils_1.batchFetch)(nonDefaults, async (chunk) => (0, codamaHelpers_1.unwrapAccounts)(await (0, accounts_4.fetchAllMaybePositionV2)(this._rpc, chunk)));
        const fetchedMap = fetched.reduce((map, position, i) => {
            map[nonDefaults[i]] = position;
            return map;
        }, {});
     ...
     */
    getWhirlpoolByAddress = async (whirlpool) => (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_2.fetchMaybeWhirlpool)(this._rpc, whirlpool));
    /**
    ...
    getRaydiumPools = async (pools) => {
        ...
        const fetched = await (0, utils_1.batchFetch)(uniquePools, async (chunk) => (0, codamaHelpers_1.unwrapAccounts)(await (0, accounts_3.fetchAllMaybePoolState)(this._rpc, chunk)));
        fetched.reduce((map, whirlpool, i) => {
    ...
    getMeteoraPools = async (pools) => {
        ...
        const fetched = await (0, utils_1.batchFetch)(uniquePools, async (chunk) => (0, codamaHelpers_1.unwrapAccounts)(await (0, accounts_4.fetchAllMaybeLbPair)(this._rpc, chunk)));
        fetched.reduce((map, whirlpool, i) => {
            poolsMap.set(uniquePools[i], whirlpool);
            return map;
        }, {});
    ...
    getRaydiumAmmConfig = async (config) => (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_3.fetchMaybeAmmConfig)(this._rpc, config));
    /**
     * Get Raydium pool from public key
     * @param pool pubkey of the orca whirlpool
     */
    getRaydiumPoolByAddress = async (pool) => (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_3.fetchMaybePoolState)(this._rpc, pool));
    getMeteoraPoolByAddress = async (pool) => (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_4.fetchMaybeLbPair)(this._rpc, pool));
    getEventAuthorityPDA = async (dex) => {
        if (Number(dex) === (0, utils_1.dexToNumber)('ORCA') || Number(dex) === (0, utils_1.dexToNumber)('RAYDIUM')) {
            return (0, kit_1.none)();
        }
        if (Number(dex) === (0, utils_1.dexToNumber)('METEORA')) {
            const [key] = await (0, kit_1.getProgramDerivedAddress)({
                seeds: [(0, bytes_1.encodeUtf8)('__event_authority')],
    ...
    withdrawShares = async (strategy, sharesAmount, owner, sharesAtaBalance) => {
        ...
        const args = { sharesAmount: BigInt(sharesAmountInLamports.floor().toString()) };
        const accounts = {
            user: owner,
        ...
        let withdrawIx = (0, instructions_2.getWithdrawInstruction)({ ...args, ...accounts }, { programAddress: this.getProgramID() });
        ...
        const isRaydium = Number(strategyState.strategy.strategyDex) === (0, utils_1.dexToNumber)('RAYDIUM');
        if (isRaydium) {
            const raydiumPosition = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_3.fetchMaybePersonalPositionState)(this._rpc, strategyState.strategy.position));
            if (!raydiumPosition) {
            ...
            const [poolTickArrayBitmap, _poolTickArrayBitmapBump] = await (0, kit_1.getProgramDerivedAddress)({
                seeds: [(0, bytes_1.encodeUtf8)('pool_tick_array_bitmap_extension'), addressEncoder.encode(strategyState.strategy.pool)],
    ...
    getTreasuryFeeVaultPDAs = async (tokenAMint, tokenBMint) => {
        const [[treasuryFeeTokenAVault], [treasuryFeeTokenBVault], [treasuryFeeVaultAuthority]] = await Promise.all([
            (0, kit_1.getProgramDerivedAddress)({
                seeds: [(0, bytes_1.encodeUtf8)('treasury_fee_vault'), addressEncoder.encode(tokenAMint)],
                programAddress: this.getProgramID(),
            }),
            (0, kit_1.getProgramDerivedAddress)({
                seeds: [(0, bytes_1.encodeUtf8)('treasury_fee_vault'), addressEncoder.encode(tokenBMint)],
                programAddress: this.getProgramID(),
            }),
            (0, kit_1.getProgramDerivedAddress)({
                seeds: [(0, bytes_1.encodeUtf8)('treasury_fee_vault_authority')],
                programAddress: this.getProgramID(),
            }),
        ]);
        return { treasuryFeeTokenAVault, treasuryFeeTokenBVault, treasuryFeeVaultAuthority };
    };
    getGlobalConfigValue = (value) => {
        if (typeof value === 'string' && (0, kit_1.isAddress)(value)) {
            return new Uint8Array((0, kit_1.getAddressEncoder)().encode(value));
        }
     ...
     * @returns transaction instruction
     */
    initializeGlobalConfig = (owner, globalConfig) => {
        return (0, instructions_2.getInitializeGlobalConfigInstruction)({
     ...
     * @returns transaction instruction
     */
    updateGlobalConfig = (owner, globalConfig, globalConfigOption, index, value) => {
        let updateConfigIx = (0, instructions_2.getUpdateGlobalConfigInstruction)({
            adminAuthority: owner,
            globalConfig,
            systemProgram: system_1.SYSTEM_PROGRAM_ADDRESS,
            key: globalConfigOption,
            index,
            value: this.getGlobalConfigValue(value),
        }, { programAddress: this.getProgramID() });
        if ((globalConfigOption === types_1.GlobalConfigOption.UpdateScopePriceId ||
            globalConfigOption === types_1.GlobalConfigOption.AddScopePriceId) &&
            typeof value === 'string' &&
            (0, kit_1.isAddress)(value)) {
            updateConfigIx = appendAccounts(updateConfigIx, [{ address: value, role: kit_1.AccountRole.READONLY }]);
        }
    ...
    getAllDepositAccounts = async (strategy, owner) => {
        const strategyState = await this.getStrategyStateIfNotFetched(strategy);
        const globalConfig = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_1.fetchMaybeGlobalConfig)(this._rpc, strategyState.strategy.globalConfig));
        if (!globalConfig) {
        ...
        accounts.push(...this.strategyTokenScopeFeedsToArray(scopePricesFeeds).slice(2));
        return accounts;
    };
    /**
    ...
    deposit = async (strategy, amountA, amountB, owner) => {
        ...
        let depositIx = (0, instructions_2.getDepositInstruction)({ ...depositArgs, ...depositAccounts }, { programAddress: this.getProgramID() });
        const rewardFeedAccounts = scopePricesFeeds.rewardsFeeds
            .filter((feed) => feed !== undefined)
            .map((address) => ({ address, role: kit_1.AccountRole.READONLY }));
    ...
    onlyDirectRoutes) => {
        ...
        return await profiler(this.getSingleSidedDepositIxs(strategyWithAddress, (0, utils_1.collToLamportsDecimal)(tokenAMinPostDepositBalance, Number(strategyWithAddress.strategy.tokenAMintDecimals)), (0, utils_1.collToLamportsDecimal)(userTokenBalances.b, Number(strategyWithAddress.strategy.tokenBMintDecimals)), owner, slippageBps, swapper, profiler, userTokenBalancesWithoutSolBalanace, priceAInB, includeAtaIxns), 'A-getSingleSidedDepositIxs', []);
    };
    ...
    onlyDirectRoutes) => {
        ...
        const swapper = swapIxsBuilder
            ? swapIxsBuilder
            : (input, tokenAMint, tokenBMint, user, slippageBps, allAccounts) => this.getJupSwapIxsV6(input, tokenAMint, tokenBMint, user.address, slippageBps, false, allAccounts, profiler, onlyDirectRoutes);
        return await profiler(this.getSingleSidedDepositIxs(strategyWithAddress, (0, utils_1.collToLamportsDecimal)(userTokenBalances.a, Number(strategyWithAddress.strategy.tokenAMintDecimals)), (0, utils_1.collToLamportsDecimal)(tokenBMinPostDepositBalance, Number(strategyWithAddress.strategy.tokenBMintDecimals)), owner, slippageBps, swapper, profiler, userTokenBalancesWithoutSolBalanace, priceAInB, includeAtaIxns), 'A-getSingleSidedDepositIxs', []);
    };
    ...
    includeAtaIxns = true // if true it includes create and close wsol and token atas,
        ...
        let aToDeposit = (0, utils_1.collToLamportsDecimal)(tokenAAtaBalance, Number(strategyState.tokenAMintDecimals)).sub(tokenAMinPostDepositBalanceLamports);
        let bToDeposit = (0, utils_1.collToLamportsDecimal)(tokenBAtaBalance, Number(strategyState.tokenBMintDecimals)).sub(tokenBMinPostDepositBalanceLamports);
        const cleanupIxs = [];
        ...
        if ((0, utils_1.isSOLMint)(strategyState.tokenAMint)) {
            ...
            const tokenAAtaBalanceLamports = (0, utils_1.collToLamportsDecimal)(tokenAAtaBalance, Number(strategyState.tokenAMintDecimals));
            ...
            const createWSolAtaIxns = await (0, transactions_1.createWsolAtaIfMissing)(this._rpc, new decimal_js_1.default((0, utils_1.lamportsToNumberDecimal)(solToDeposit, utils_1.DECIMALS_SOL)), owner, 'deposit', this.logger);
        ...
        if ((0, utils_1.isSOLMint)(strategyState.tokenBMint)) {
            const solBalance = new decimal_js_1.default((await this._rpc.getBalance(owner.address).send()).value.toString());
            const tokenBAtaBalanceLamports = (0, utils_1.collToLamportsDecimal)(tokenBAtaBalance, Number(strategyState.tokenBMintDecimals));
        ...
        const singleSidedDepositIx = (0, instructions_2.getSingleTokenDepositWithMinInstruction)({ ...args, ...accounts }, { programAddress: this.getProgramID() });
        let singleSidedDepositIxWithRemainingAccounts = singleSidedDepositIx;
        const rewardFeedAccounts = scopePricesFeeds.rewardsFeeds
            .filter((feed) => feed !== undefined)
            .map((address) => ({ address, role: kit_1.AccountRole.READONLY }));
        ...
        if (rewardFeedAccounts.length > 0 || kaminoRewardFeedAccounts.length > 0) {
            singleSidedDepositIxWithRemainingAccounts = appendAccounts(singleSidedDepositIx, [
                ...rewardFeedAccounts,
                ...kaminoRewardFeedAccounts,
            ]);
        }
        let result = [];
        if (includeAtaIxns) {
    ...
    withdrawTopupVault = async (owner, amount) => {
        ...
        const withdrawIxn = (0, instructions_2.getWithdrawFromTopupInstruction)({ ...args, ...accounts }, { programAddress: this.getProgramID() });
        return withdrawIxn;
    };
    getJupSwapIxsWithMaxAccounts = async (input, tokenAMint, tokenBMint, owner, slippageBps, useOnlyLegacyTransaction, existingAccounts, maxAccounts, profiler = utils_1.noopProfiledFunctionExecution, onlyDirectRoutes) => {
        const jupiterQuote = input.tokenAToSwapAmount.lt(utils_1.ZERO)
            ? await profiler(JupService_1.JupService.getBestRouteV6(owner, input.tokenAToSwapAmount.abs(), tokenAMint, tokenBMint, slippageBps.toNumber(), useOnlyLegacyTransaction, maxAccounts, onlyDirectRoutes, this.logger), 'C-getBestRouteV6', [])
            : await profiler(JupService_1.JupService.getBestRouteV6(owner, input.tokenBToSwapAmount.abs(), tokenBMint, tokenAMint, slippageBps.toNumber(), useOnlyLegacyTransaction, maxAccounts, onlyDirectRoutes, this.logger), 'C-getBestRouteV6', []);
        const allJupIxs = [
        ...
        this.logger.info(`${prefix} All distinct existing accounts number ${new Set(existingAccounts).size}`);
        this.logger.info(`${prefix} All distinct Jup accounts number ${new Set(allJupAccounts).size}`);
    ...
    getCheckExpectedVaultsBalancesIx = async (strategy, user, tokenAAta, tokenBAta, expectedTokensBalances) => {
        ...
        const expectedALamportsDecimal = (0, utils_1.collToLamportsDecimal)(expectedABalance, Number(strategyState.tokenAMintDecimals));
        const expectedBLamportsDecimal = (0, utils_1.collToLamportsDecimal)(expectedBBalance, Number(strategyState.tokenBMintDecimals));
        ...
        return (0, instructions_2.getCheckExpectedVaultsBalancesInstruction)({ ...args, ...accounts }, { programAddress: this.getProgramID() });
    };
    /**
    ...
    createStrategy = async (strategy, pool, owner, dex) => {
        ...
        if (dex === 'ORCA') {
            const whirlpoolState = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_2.fetchMaybeWhirlpool)(this._rpc, pool));
            if (!whirlpoolState) {
            ...
            const raydiumPoolState = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_3.fetchMaybePoolState)(this._rpc, pool));
            if (!raydiumPoolState) {
            ...
            const meteoraPoolState = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_4.fetchMaybeLbPair)(this._rpc, pool));
            if (!meteoraPoolState) {
        ...
        const strategyArgs = {
            ...
            strategyType: BigInt((0, utils_1.dexToNumber)(dex)),
        };
        ...
        return (0, instructions_2.getInitializeStrategyInstruction)({ ...strategyArgs, ...strategyAccounts }, { programAddress: this.getProgramID() });
    };
    /**
    ...
    closeStrategy = async (admin, strategy) => {
        ...
        let ix = (0, instructions_2.getCloseStrategyInstruction)({ ...strategyAccounts }, { programAddress: this.getProgramID() });
        for (let i = 0; i < 6; i++) {
    ...
    getUserTopupVault = async (user) => {
        const [topupVault] = await (0, kit_1.getProgramDerivedAddress)({
            seeds: [(0, bytes_1.encodeUtf8)('topup_vault'), addressEncoder.encode(user)],
            programAddress: this.getProgramID(),
        });
        return topupVault;
    };
    ...
    getStrategyProgramAddresses = async (strategy, tokenMintA, tokenMintB) => {
        const [tokenAVault, tokenABump] = await (0, kit_1.getProgramDerivedAddress)({
            seeds: [(0, bytes_1.encodeUtf8)('svault_a'), addressEncoder.encode(strategy)],
            programAddress: this.getProgramID(),
        });
        const [tokenBVault, tokenBBump] = await (0, kit_1.getProgramDerivedAddress)({
            seeds: [(0, bytes_1.encodeUtf8)('svault_b'), addressEncoder.encode(strategy)],
            programAddress: this.getProgramID(),
        });
        const [baseVaultAuthority, baseVaultAuthorityBump] = await (0, kit_1.getProgramDerivedAddress)({
            seeds: [(0, bytes_1.encodeUtf8)('authority'), addressEncoder.encode(tokenAVault), addressEncoder.encode(tokenBVault)],
            programAddress: this.getProgramID(),
        });
        const [sharesMint, sharesMintBump] = await (0, kit_1.getProgramDerivedAddress)({
            seeds: [
                (0, bytes_1.encodeUtf8)('shares'),
        ...
        const [sharesMintAuthority, sharesMintAuthorityBump] = await (0, kit_1.getProgramDerivedAddress)({
            seeds: [(0, bytes_1.encodeUtf8)('authority'), addressEncoder.encode(sharesMint)],
            programAddress: this.getProgramID(),
        });
        return {
            sharesMintAuthority,
    ...
    createStrategyAccount = async (payer, newStrategy) => {
        const accountSize = BigInt((0, accounts_1.getWhirlpoolStrategySize)());
    ...
    collectFeesAndRewards = async (strategy, owner) => {
        ...
        if (Number(strategyState.strategyDex) === (0, utils_1.dexToNumber)('ORCA')) {
            const whirlpool = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_2.fetchMaybeWhirlpool)(this._rpc, strategyState.pool));
            if (!whirlpool) {
        ...
        else if (Number(strategyState.strategyDex) === (0, utils_1.dexToNumber)('RAYDIUM')) {
            programId = this._raydiumService.getRaydiumProgramId();
            const poolState = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_3.fetchMaybePoolState)(this._rpc, strategyState.pool));
            if (!poolState) {
        ...
        else if (Number(strategyState.strategyDex) === (0, utils_1.dexToNumber)('METEORA')) {
            programId = this._meteoraService.getMeteoraProgramId();
            const poolState = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_4.fetchMaybeLbPair)(this._rpc, strategyState.pool));
            if (!poolState) {
        ...
        let ix = (0, instructions_2.getCollectFeesAndRewardsInstruction)({ ...accounts }, { programAddress: this.getProgramID() });
        const pairs = [
        ...
        if (Number(strategyState.strategyDex) === (0, utils_1.dexToNumber)('RAYDIUM')) {
            const [poolTickArrayBitmap] = await (0, kit_1.getProgramDerivedAddress)({
                seeds: [(0, bytes_1.encodeUtf8)('pool_tick_array_bitmap_extension'), addressEncoder.encode(strategyState.pool)],
                programAddress: this._raydiumService.getRaydiumProgramId(),
            });
            ix = appendAccounts(ix, [{ address: poolTickArrayBitmap, role: kit_1.AccountRole.READONLY }]);
        }
        return ix;
    };
    /**
    ...
    getMetadataProgramAddressesOrca = async (positionMint) => {
        const [position, positionBump] = await (0, kit_1.getProgramDerivedAddress)({
            seeds: [(0, bytes_1.encodeUtf8)('position'), addressEncoder.encode(positionMint)],
        ...
        const [positionMetadata, positionMetadataBump] = await (0, kit_1.getProgramDerivedAddress)({
            seeds: [(0, bytes_1.encodeUtf8)('metadata'), addressEncoder.encode(constants_1.METADATA_PROGRAM_ID), addressEncoder.encode(positionMint)],
    ...
    getStartEndTickIndexProgramAddressesOrca = async (whirlpool, whirlpoolState, tickLowerIndex, tickUpperIndex) => {
        ...
        const [lowerTickPubkey, lowerTickBump] = await (0, kit_1.getProgramDerivedAddress)({
            seeds: [(0, bytes_1.encodeUtf8)('tick_array'), addressEncoder.encode(whirlpool), (0, bytes_1.encodeUtf8)(startTickIndex.toString())],
            programAddress: this._orcaService.getWhirlpoolProgramId(),
        });
        const [upperTickPubkey, upperTickBump] = await (0, kit_1.getProgramDerivedAddress)({
            seeds: [(0, bytes_1.encodeUtf8)('tick_array'), addressEncoder.encode(whirlpool), (0, bytes_1.encodeUtf8)(endTickIndex.toString())],
            programAddress: this._orcaService.getWhirlpoolProgramId(),
        });
        return {
    ...
    getStartEndTicketIndexProgramAddressesRaydium = async (pool, poolState, tickLowerIndex, tickUpperIndex) => {
        ...
        const [lowerTickPubkey, lowerTickBump] = await (0, kit_1.getProgramDerivedAddress)({
            seeds: [(0, bytes_1.encodeUtf8)('tick_array'), addressEncoder.encode(pool), (0, lib_1.i32ToBytes)(startTickIndex)],
            programAddress: this._raydiumService.getRaydiumProgramId(),
        });
        const [upperTickPubkey, upperTickBump] = await (0, kit_1.getProgramDerivedAddress)({
            seeds: [(0, bytes_1.encodeUtf8)('tick_array'), addressEncoder.encode(pool), (0, lib_1.i32ToBytes)(endTickIndex)],
            programAddress: this._raydiumService.getRaydiumProgramId(),
        });
        return {
            lowerTick: lowerTickPubkey,
    ...
    readMeteoraPosition = async (poolPk, positionPk, pool, position) => {
        pool = pool ?? (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_4.fetchMaybeLbPair)(this._rpc, poolPk));
        position = position ?? (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_4.fetchMaybePositionV2)(this._rpc, positionPk));
        if (!pool || !position) {
            return {
                Address: positionPk,
        ...
        const binArray = (0, codamaHelpers_1.unwrapAccounts)(await (0, accounts_4.fetchAllMaybeBinArray)(this._rpc, [lowerTickPk, upperTickPk]));
    ...
    getStartEndTicketIndexProgramAddressesMeteora = async (pool, tickLowerIndex) => {
        ...
        const lowerBinArrayIndex = (0, meteora_1.binIdToBinArrayIndex)(BigInt(tickLowerIndex));
     ...
     */
    openPosition = async (admin, strategy, positionMint, priceLower, priceUpper, status = strategyStatus_1.StrategyStatus.Uninitialized) => {
    ...
    openPositionOrca = async (adminAuthority, strategy, baseVaultAuthority, pool, positionMint, priceLower, priceUpper, tokenAVault, tokenBVault, tokenAMint, tokenBMint, tokenATokenProgram, tokenBTokenProgram, oldPositionOrBaseVaultAuthority, oldPositionMintOrBaseVaultAuthority, oldPositionTokenAccountOrBaseVaultAuthority, oldTickArrayLowerOrBaseVaultAuthority, oldTickArrayUpperOrBaseVaultAuthority, status = strategyStatus_1.StrategyStatus.Uninitialized) => {
        ...
        const ix = (0, instructions_2.getOpenLiquidityPositionInstruction)({ ...args, ...accounts }, { programAddress: this.getProgramID() });
    ...
    openPositionRaydium = async (adminAuthority, strategy, baseVaultAuthority, pool, positionMint, priceLower, priceUpper, tokenAVault, tokenBVault, tokenAMint, tokenBMint, tokenATokenProgram, tokenBTokenProgram, oldTickArrayLowerOrBaseVaultAuthority, oldTickArrayUpperOrBaseVaultAuthority, oldPositionOrBaseVaultAuthority, oldPositionMintOrBaseVaultAuthority, oldPositionTokenAccountOrBaseVaultAuthority, oldProtocolPositionOrBaseVaultAuthority, eventAuthority, status = strategyStatus_1.StrategyStatus.Uninitialized, strategyRewardOVault, strategyReward1Vault, strategyReward2Vault) => {
        ...
        const [poolTickArrayBitmap] = await (0, kit_1.getProgramDerivedAddress)({
            seeds: [(0, bytes_1.encodeUtf8)('pool_tick_array_bitmap_extension'), addressEncoder.encode(pool)],
            programAddress: this._raydiumService.getRaydiumProgramId(),
        });
        let ix = (0, instructions_2.getOpenLiquidityPositionInstruction)({ ...args, ...accounts }, { programAddress: this.getProgramID() });
        ix = appendAccounts(ix, [
     ...
     * @param status strategy status
     */
    openPositionMeteora = async (adminAuthority, strategy, baseVaultAuthority, pool, position, priceLower, priceUpper, tokenAVault, tokenBVault, tokenAMint, tokenBMint, tokenATokenProgram, tokenBTokenProgram, oldPositionOrBaseVaultAuthority, oldPositionMintOrBaseVaultAuthority, oldPositionTokenAccountOrBaseVaultAuthority, oldTickArrayLowerOrBaseVaultAuthority, oldTickArrayUpperOrBaseVaultAuthority, eventAuthority, status = strategyStatus_1.StrategyStatus.Uninitialized) => {
        const lbPair = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_4.fetchMaybeLbPair)(this._rpc, pool));
        if (!lbPair) {
    ...
    executiveWithdraw = async (admin, strategy, action) => {
        ...
        let executiveWithdrawIx = (0, instructions_2.getExecutiveWithdrawInstruction)({ ...args, ...accounts }, { programAddress: this.getProgramID() });
        //  for Raydium strats we need to collect fees and rewards before withdrawal
        //  add rewards vaults accounts to withdraw
        const isRaydium = Number(strategyState.strategyDex) === (0, utils_1.dexToNumber)('RAYDIUM');
        if (isRaydium) {
    ...
    getUpdateReferencePriceTypeIx = async (admin, strategy, referencePriceType) => {
        ...
        return (0, utils_1.getUpdateStrategyConfigIx)(admin, this._globalConfig, address, types_1.StrategyConfigOption.UpdateReferencePriceType, new decimal_js_1.default(referencePriceType), this.getProgramID());
    };
    /**
    ...
    invest = async (strategy, payer) => {
        ...
        return (0, instructions_2.getInvestInstruction)({ ...accounts }, { programAddress: this.getProgramID() });
    };
    /**
    ...
    getUpdateRebalancingParamsFromRebalanceFieldsIx = async (strategyAdmin, strategy, rebalanceFieldInfos) => {
        ...
        return this.getUpdateRebalancingParamsIxns(strategyAdmin, strategyWithAddress.address, rebalanceParams, rebalanceType, Number(strategyWithAddress.strategy.tokenAMintDecimals), Number(strategyWithAddress.strategy.tokenBMintDecimals));
    };
    processRebalanceParams = async (dex, pool, rebalanceType, rebalanceParams) => {
        ...
        if (dex === 'ORCA') {
            ...
            if (rebalanceTypeKind === types_1.RebalanceType.Drift) {
    ...
    getUpdateRebalancingParamsIxns = async (strategyAdmin, strategy, rebalanceParams, rebalanceType, tokenADecimals, tokenBDecimals) => {
        ...
        const processedRebalanceParams = await this.processRebalanceParams((0, utils_1.numberToDex)(Number(strategyState.strategyDex)), strategyState.pool, new decimal_js_1.default(rebalanceType), rebalanceParams);
        const value = (0, utils_1.buildStrategyRebalanceParams)(processedRebalanceParams, rebalanceType, tokenADecimals, tokenBDecimals);
        const args = {
            mode: types_1.StrategyConfigOption.UpdateRebalanceParams,
        ...
        return (0, instructions_2.getUpdateStrategyConfigInstruction)({ ...args, ...accounts }, { programAddress: this.getProgramID() });
    };
    ...
    buildStrategyRebalanceParams = async (strategy) => {
        const strategyState = await this.getStrategyStateIfNotFetched(strategy);
        const params = Array.from(strategyState.strategy.rebalanceRaw.params).map((p) => new decimal_js_1.default(p.toString()));
    ...
    getBuildStrategyIxns = async (dex, feeTierBps, strategy, positionMint, strategyAdmin, rebalanceType, rebalanceParams, tokenAMint, tokenBMint, depositCap, depositCapPerIx, withdrawFeeBps, depositFeeBps, performanceFeeBps) => {
        ...
        const eventAuthority = unwrapOption(await this.getEventAuthorityPDA(BigInt((0, utils_1.dexToNumber)(dex))));
        if (dex === 'ORCA') {
    ...
    async getNewPositionRange(strategy, rebalanceKind, rebalanceParams) {
        const strategyState = await this.getStrategyStateIfNotFetched(strategy);
        const dex = (0, utils_1.numberToDex)(Number(strategyState.strategy.strategyDex));
    ...
    async getRebalancePositionRange(dex, price, tokenAMint, tokenBMint, tickSpacing, rebalanceKind, rebalanceParams) {
        const tokenADecimals = await (0, utils_1.getMintDecimals)(this._rpc, tokenAMint);
        const tokenBDecimals = await (0, utils_1.getMintDecimals)(this._rpc, tokenBMint);
        switch (rebalanceKind) {
            case types_1.RebalanceType.Manual:
                return { lowerPrice: rebalanceParams[0], upperPrice: rebalanceParams[1] };
            case types_1.RebalanceType.TakeProfit:
                return { lowerPrice: rebalanceParams[0], upperPrice: rebalanceParams[1] };
            case types_1.RebalanceType.PricePercentage:
                return (0, rebalance_methods_1.getPositionRangeFromPercentageRebalanceParams)(price, rebalanceParams[0], rebalanceParams[1]);
            case types_1.RebalanceType.PricePercentageWithReset:
            ...
            case types_1.RebalanceType.Drift:
    ...
    async getCurrentPrice(strategy) {
        ...
        const dex = (0, utils_1.numberToDex)(Number(strategyWithAddress.strategy.strategyDex));
        return this.getCurrentPriceFromPool(dex, pool, Number(strategyWithAddress.strategy.tokenAMintDecimals), Number(strategyWithAddress.strategy.tokenBMintDecimals));
    }
    ...
    async readRebalancingParamsFromChain(strategy) {
        ...
        let rebalanceFields;
        if (rebalanceKind === types_1.RebalanceType.Manual) {
            rebalanceFields = [];
        }
        else if (rebalanceKind === types_1.RebalanceType.PricePercentage) {
        ...
        else if (rebalanceKind === types_1.RebalanceType.PricePercentageWithReset) {
        ...
        else if (rebalanceKind === types_1.RebalanceType.Drift) {
        ...
        else if (rebalanceKind === types_1.RebalanceType.TakeProfit) {
        ...
        else if (rebalanceKind === types_1.RebalanceType.PeriodicRebalance) {
        ...
        else if (rebalanceKind === types_1.RebalanceType.Autodrift) {
    ...
    async getStrategyCurrentWithdrawalCaps(strategy) {
        ...
        return {
            a: (0, utils_1.lamportsToNumberDecimal)(tokenAWithdrawalCapTokens, Number(strategyWithAddress.strategy.tokenAMintDecimals)),
            b: (0, utils_1.lamportsToNumberDecimal)(tokenBWithdrawalCapTokens, Number(strategyWithAddress.strategy.tokenBMintDecimals)),
        };
    }
    /**
    ...
    async getRaydiumPoolPrice(pool) {
        ...
        const price = lib_1.SqrtPriceMath.sqrtPriceX64ToPrice((0, raydiumBridge_1.toBN)(poolState.sqrtPriceX64), poolState.mintDecimals0, poolState.mintDecimals1);
        return price;
    }
    async getMeteoraPoolPrice(pool) {
        const poolState = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_4.fetchMaybeLbPair)(this._rpc, pool));
        if (!poolState) {
    ...
    setupStrategyLookupTable = async (authority, strategy, slot) => {
        ...
        const updateStrategyLookupTableIx = await (0, utils_1.getUpdateStrategyConfigIx)(authority, this._globalConfig, strategyPk, types_1.StrategyConfigOption.UpdateLookupTable, new decimal_js_1.default(0), this.getProgramID(), lookupTable);
        return {
            lookupTable,
            createLookupTableIx,
    ...
    rebalance = async (admin, strategy, newPosition, priceLower, priceUpper) => {
        ...
        if (Number(strategyWithAddress.strategy.strategyDex) === (0, utils_1.dexToNumber)('ORCA') ||
        ...
        ixs.push(await this.openPosition(admin, strategyWithAddress, newPosition, priceLower, priceUpper, strategyStatus_1.StrategyStatus.Rebalancing));
        return ixs;
    };
    /**
            ...
            if ('parsed' in accountData) {
                ...
                if (strategy) {
                    positions.push({
                        ...
                        strategyDex: (0, utils_1.numberToDex)(Number(strategy.strategy.strategyDex)),
                    });
                }
            }
        }
    ...
    getUserPositionsByStrategiesMap = async (wallet, strategiesWithShareMintsMap, strategiesWithAddressMap) => {
        ...
        for (let i = 0; i < mints.length; i++) {
            ...
            const [expectedMintAuthority] = await (0, kit_1.getProgramDerivedAddress)({
                seeds: [(0, bytes_1.encodeUtf8)('authority'), addressEncoder.encode(mint)],
                programAddress: this.getProgramID(),
            });
        ...
        for (const index of strategies.keys()) {
            ...
            positions.push({
                ...
                strategyDex: (0, utils_1.numberToDex)(Number(strategy.strategyDex)),
            });
        }
        return positions;
    };
    ...
    calculateAmountsDistributionWithPriceRange = async (dex, pool, lowerPrice, upperPrice) => {
        ...
        if (dex === 'RAYDIUM') {
            ...
            const { amountA, amountB } = lib_1.LiquidityMath.getAmountsFromLiquidity((0, raydiumBridge_1.toBN)(poolState.sqrtPriceX64), lib_1.SqrtPriceMath.priceToSqrtPriceX64(lowerPrice, decimalsA, decimalsB), lib_1.SqrtPriceMath.priceToSqrtPriceX64(upperPrice, decimalsA, decimalsB), new bn_js_1.default(100_000_000), true);
    ...
    calculateAmountsOrca = async ({ whirlpoolState, positionAddress, tokenAAmount, tokenBAmount, }) => {
        ...
        const positionState = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_2.fetchMaybePosition)(this._rpc, positionAddress));
        if (!positionState) {
    ...
    calculateAmountsRaydium = async ({ strategyState, tokenAAmount, tokenBAmount, }) => {
        ...
        const poolState = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_3.fetchMaybePoolState)(this._rpc, strategyState.pool));
        const position = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_3.fetchMaybePersonalPositionState)(this._rpc, strategyState.position));
        if (!position) {
        ...
        if (tokenAAmount && tokenBAmount && tokenAAmount.gt(0) && tokenBAmount.gt(0)) {
            const liquidity = lib_1.LiquidityMath.getLiquidityFromTokenAmounts((0, raydiumBridge_1.toBN)(poolState.sqrtPriceX64), lib_1.SqrtPriceMath.getSqrtPriceX64FromTick(position.tickLowerIndex), lib_1.SqrtPriceMath.getSqrtPriceX64FromTick(position.tickUpperIndex), new bn_js_1.default(tokenAAmount.toString()), new bn_js_1.default(tokenBAmount.toString()));
            const { amountA, amountB } = lib_1.LiquidityMath.getAmountsFromLiquidity((0, raydiumBridge_1.toBN)(poolState.sqrtPriceX64), lib_1.SqrtPriceMath.getSqrtPriceX64FromTick(position.tickLowerIndex), lib_1.SqrtPriceMath.getSqrtPriceX64FromTick(position.tickUpperIndex), liquidity, true);
            ...
            const { amountA, amountB } = lib_1.LiquidityMath.getAmountsFromLiquidity((0, raydiumBridge_1.toBN)(poolState.sqrtPriceX64), lib_1.SqrtPriceMath.getSqrtPriceX64FromTick(position.tickLowerIndex), lib_1.SqrtPriceMath.getSqrtPriceX64FromTick(position.tickUpperIndex), 
    ...
    calculateAmountsMeteora = async ({ strategyState, tokenAAmount, tokenBAmount, }) => {
        ...
        if (tokenAAmount && tokenBAmount && tokenAAmount.gt(0) && tokenBAmount.gt(0)) {
            ...
            const binArrayIndex = (0, meteora_1.binIdToBinArrayIndex)(BigInt(poolState.activeId));
            ...
            const binArray = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_4.fetchMaybeBinArray)(this._rpc, binArrayPk));
            if (!binArray) {
    ...
    async getUserTermsSignatureState(owner) {
        const pdaSeed = [(0, bytes_1.encodeUtf8)('signature'), addressEncoder.encode(owner)];
        const [signatureStateKey, _signatureStateBump] = await (0, kit_1.getProgramDerivedAddress)({
            seeds: pdaSeed,
            programAddress: this.getProgramID(),
        });
        return (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_1.fetchMaybeTermsSignature)(this._rpc, signatureStateKey));
    }
    /**
    ...
    async getUserTermsSignatureIx(owner, signature) {
        const pdaSeed = [(0, bytes_1.encodeUtf8)('signature'), addressEncoder.encode(owner.address)];
        ...
        return (0, instructions_2.getSignTermsInstruction)({ ...args, ...accounts }, { programAddress: this.getProgramID() });
    }
    async getDepositRatioFromAOrca(strategy, amountA) {
        ...
        const position = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_2.fetchMaybePosition)(this._rpc, strategyState.position));
        if (!position) {
    ...
    async getDepositRatioFromAMeteora(strategy) {
        const { strategy: strategyState } = await this.getStrategyStateIfNotFetched(strategy);
        const poolStatePromise = (0, accounts_4.fetchMaybeLbPair)(this._rpc, strategyState.pool).then(codamaHelpers_1.unwrapAccount);
        const positionPromise = (0, accounts_4.fetchMaybePositionV2)(this._rpc, strategyState.position).then(codamaHelpers_1.unwrapAccount);
    ...
    async getDepositRatioFromBMeteora(strategy) {
        ...
        const [poolState, _position] = await Promise.all([
            (0, accounts_4.fetchMaybeLbPair)(this._rpc, strategyState.pool).then(codamaHelpers_1.unwrapAccount),
            (0, accounts_4.fetchMaybePositionV2)(this._rpc, strategyState.position).then(codamaHelpers_1.unwrapAccount),
        ]);
        if (!poolState) {
            throw Error(`Could not fetch lb pair state with pubkey ${strategyState.pool.toString()}`);
        }
    ...
    getDepositRatioFromARaydium = async (strategy, amountA) => {
        const { strategy: strategyState } = await this.getStrategyStateIfNotFetched(strategy);
        const poolState = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_3.fetchMaybePoolState)(this._rpc, strategyState.pool));
        const positionState = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_3.fetchMaybePersonalPositionState)(this._rpc, strategyState.position));
        if (!positionState) {
        ...
        const liqudity = lib_1.LiquidityMath.getLiquidityFromTokenAmountA(lowerSqrtPriceX64, upperSqrtPriceX64, (0, raydiumBridge_1.toBN)(amountA), false);
        const amountsSlippage = lib_1.LiquidityMath.getAmountsFromLiquidityWithSlippage((0, raydiumBridge_1.toBN)(poolState.sqrtPriceX64), lowerSqrtPriceX64, upperSqrtPriceX64, liqudity, true, false, 1);
        return {
            amountSlippageA: (0, raydiumBridge_1.fromBN)(amountsSlippage.amountSlippageA),
            amountSlippageB: (0, raydiumBridge_1.fromBN)(amountsSlippage.amountSlippageB),
        };
    };
    getDepositRatioFromBRaydium = async (strategy, amountB) => {
        ...
        const lowerSqrtPriceX64 = lib_1.SqrtPriceMath.getSqrtPriceX64FromTick(positionState.tickLowerIndex);
        const upperSqrtPriceX64 = lib_1.SqrtPriceMath.getSqrtPriceX64FromTick(positionState.tickUpperIndex);
        const liqudity = lib_1.LiquidityMath.getLiquidityFromTokenAmountB(lowerSqrtPriceX64, upperSqrtPriceX64, (0, raydiumBridge_1.toBN)(amountB));
        const amountsSlippage = lib_1.LiquidityMath.getAmountsFromLiquidityWithSlippage((0, raydiumBridge_1.toBN)(poolState.sqrtPriceX64), lowerSqrtPriceX64, upperSqrtPriceX64, liqudity, true, false, 1);
        return {
            amountSlippageA: (0, raydiumBridge_1.fromBN)(amountsSlippage.amountSlippageA),
            amountSlippageB: (0, raydiumBridge_1.fromBN)(amountsSlippage.amountSlippageB),
    ...
    getCollateralInfo = async (address) => {
        const collateralInfos = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_1.fetchMaybeCollateralInfos)(this._rpc, address));
    ...
    getGlobalConfigState = async (address) => {
        return (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_1.fetchMaybeGlobalConfig)(this._rpc, address));
    };
    getWhirlpoolStrategy = async (address) => {
        return (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_1.fetchMaybeWhirlpoolStrategy)(this._rpc, address));
    };
    getWhirlpoolStrategies = async (addresses) => {
        return (0, codamaHelpers_1.unwrapAccounts)(await (0, accounts_1.fetchAllMaybeWhirlpoolStrategy)(this._rpc, addresses));
    };
    ...
    getUpdateStrategyParamsIxs = async (strategyAdmin, strategy, rebalanceType, withdrawFeeBps, performanceFeeBps) => {
        const updateRebalanceTypeIx = await (0, utils_1.getUpdateStrategyConfigIx)(strategyAdmin, this._globalConfig, strategy, types_1.StrategyConfigOption.UpdateRebalanceType, rebalanceType, this.getProgramID());
        let updateWithdrawalFeeIx = null;
        if (withdrawFeeBps) {
            updateWithdrawalFeeIx = await (0, utils_1.getUpdateStrategyConfigIx)(strategyAdmin, this._globalConfig, strategy, types_1.StrategyConfigOption.UpdateWithdrawFee, withdrawFeeBps, this.getProgramID());
        }
        if (!performanceFeeBps) {
            performanceFeeBps = DefaultStrategyConfig_1.DefaultPerformanceFeeBps;
        }
        const updateFeesFeeIx = await (0, utils_1.getUpdateStrategyConfigIx)(strategyAdmin, this._globalConfig, strategy, types_1.StrategyConfigOption.UpdateCollectFeesFee, performanceFeeBps, this.getProgramID());
        const updateRewards0FeeIx = await (0, utils_1.getUpdateStrategyConfigIx)(strategyAdmin, this._globalConfig, strategy, types_1.StrategyConfigOption.UpdateReward0Fee, performanceFeeBps, this.getProgramID());
        const updateRewards1FeeIx = await (0, utils_1.getUpdateStrategyConfigIx)(strategyAdmin, this._globalConfig, strategy, types_1.StrategyConfigOption.UpdateReward1Fee, performanceFeeBps, this.getProgramID());
        const updateRewards2FeeIx = await (0, utils_1.getUpdateStrategyConfigIx)(strategyAdmin, this._globalConfig, strategy, types_1.StrategyConfigOption.UpdateReward2Fee, performanceFeeBps, this.getProgramID());
        ...
        if (Number(strategyState.strategyDex) === (0, utils_1.dexToNumber)('ORCA')) {
            ...
            for (let i = 0; i < 3; i++) {
                if (whirlpool.rewardInfos[i].mint !== pubkeys_1.DEFAULT_PUBLIC_KEY) {
                    ...
                    const ix = (0, instructions_2.getUpdateRewardMappingInstruction)({ ...args, ...accounts }, { programAddress: this.getProgramID() });
    ...
    initializeTickForOrcaPool = async (payer, pool, price) => {
        ...
        const tick = (0, codamaHelpers_1.unwrapAccount)(await (0, accounts_2.fetchMaybeTickArray)(this._rpc, startTickIndexPk));
        // initialize tick if it doesn't exist
        if (!tick) {
            return {
                tick: startTickIndexPk,
                initTickIx: (0, instructions_1.getInitializeTickArrayInstruction)({
                    startTickIndex,
    ...
    initializeTickForMeteoraPool = async (payer, pool, price) => {
        ...
        const binArrayIndex = (0, meteora_1.binIdToBinArrayIndex)(BigInt(binArray));
        ...
        if (!tick) {
            return {
                tick: startTickIndexPk,
                initTickIx: (0, instructions_3.getInitializeBinArrayInstruction)({
                    index: binArrayIndex,
                    funder: payer,
