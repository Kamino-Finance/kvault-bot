...
function readDriftRebalanceParamsFromStrategy(rebalanceRaw) {
    ...
    const params = {};
    params['startMidTick'] = new decimal_js_1.default(dv.getInt32(0, true));
    ...
    params['secondsPerTick'] = new decimal_js_1.default(dv.getBigUint64(12, true).toString());
    params['direction'] = new decimal_js_1.default(dv.getUint8(20));
    return params;
}
function readRawDriftRebalanceStateFromStrategy(rebalanceRaw) {
    ...
    const dv = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
    const state = {};
    state['step'] = new decimal_js_1.default(dv.getUint8(0));
    state['lastDriftTimestamp'] = new decimal_js_1.default(dv.getBigUint64(1, true).toString());
    state['lastMidTick'] = new decimal_js_1.default(dv.getInt32(9, true));
    return state;
}
function readDriftRebalanceStateFromStrategy(dex, tickSpacing, tokenADecimals, tokenBDecimals, rebalanceRaw) {
    const stateBuf = new Uint8Array(rebalanceRaw.state);
    const paramsBuf = new Uint8Array(rebalanceRaw.params);
    const sdv = new DataView(stateBuf.buffer, stateBuf.byteOffset, stateBuf.byteLength);
    const pdv = new DataView(paramsBuf.buffer, paramsBuf.byteOffset, paramsBuf.byteLength);
    const lastMidTick = new decimal_js_1.default(sdv.getInt32(9, true));
    const ticksBelowMid = new decimal_js_1.default(pdv.getInt32(4, true));
    const ticksAboveMid = new decimal_js_1.default(pdv.getInt32(8, true));
