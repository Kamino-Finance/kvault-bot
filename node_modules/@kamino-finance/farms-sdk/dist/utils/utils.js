...
const farms_1 = require("../@codegen/farms/errors/farms");
    ...
    try {
        ...
        if (isCustomProgramError) {
            if (!isNaN(Number(errorCode))) {
                const errorMessage = (0, farms_1.getFarmsErrorMessage)(Number(errorCode));
                if (errorMessage) {
...
async function getTokenAccountBalanceLamports(rpc, tokenAccount) {
    const tokenAccountBalance = await rpc
        .getTokenAccountBalance(tokenAccount)
        .send();
    return new decimal_js_1.Decimal(tokenAccountBalance.value.amount).toNumber();
}
...
async function fetchFarmStateWithRetry(rpc, addr) {
    return fetchWithRetry(async () => {
        const account = await (0, accounts_1.fetchMaybeFarmState)(rpc, addr);
        return account.exists ? account.data : null;
    }, addr);
}
async function fetchGlobalConfigWithRetry(rpc, addr) {
    const result = await fetchWithRetry(async () => {
        const account = await (0, accounts_1.fetchMaybeGlobalConfig)(rpc, addr);
        return account.exists ? account.data : null;
    }, addr);
    if (result === null) {
...
async function fetchUserStateWithRetry(rpc, addr) {
    const result = await fetchWithRetry(async () => {
        const account = await (0, accounts_1.fetchMaybeUserState)(rpc, addr);
        return account.exists ? account.data : null;
    }, addr);
    if (result === null) {
...
async function createKeypairRentExemptIx(rpc, payer, account, size, programId = programs_1.FARMS_PROGRAM_ADDRESS) {
