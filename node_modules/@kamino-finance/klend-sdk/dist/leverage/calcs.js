...
exports.depositLeverageCalcsDebtFlash = exports.depositLeverageCalcs = exports.estimateDepositMode = exports.estimateAdjustMode = exports.estimateWithdrawMode = exports.calcBorrowAmount = exports.LeverageOption = void 0;
...
exports.calcCollFlashLegLamports = calcCollFlashLegLamports;
...
exports.withdrawLeverageCalcsCollFlash = withdrawLeverageCalcsCollFlash;
exports.adjustDepositLeverageCalcsDebtFlash = adjustDepositLeverageCalcsDebtFlash;
exports.adjustWithdrawLeverageCalcsCollFlash = adjustWithdrawLeverageCalcsCollFlash;
...
async function calculateMultiplyEffects(getPriceByTokenMintDecimal, { depositAmount, withdrawAmount, deposited, borrowed, debtTokenMint, selectedTokenMint, collTokenMint, targetLeverage, activeLeverageOption, flashLoanFeeRatio, slippagePct, debtBorrowFactorPct, priceCollToDebt, priceDebtToColl, }, logEstimations = false) {
...
function calcWithdrawAmounts(params) {
    ...
    (0, swap_calcs_1.assertPositiveFiniteDecimal)('calcWithdrawAmounts: priceCollToDebt', priceCollToDebt);
    // MSOL/SOL
...
function calcAdjustAmounts({ targetLeverage, currentBorrowPosition, currentDepositPosition, priceCollToDebt, flashLoanFee, }) {
    (0, swap_calcs_1.assertPositiveFiniteDecimal)('calcAdjustAmounts: priceCollToDebt', priceCollToDebt);
...
const estimateDepositMode = ({ priceCollToDebt, priceDebtToColl, amount, targetLeverage, selectedTokenMint, collTokenMint, flashLoanFee, slippagePct, }) => {
...
const depositLeverageCalcs = (props) => {
    ...
    (0, swap_calcs_1.assertPositiveFiniteDecimal)('depositLeverageCalcs: priceDebtToColl', priceDebtToColl);
    // Core logic
...
function withdrawLeverageCalcs(market, collReserve, debtReserve, priceCollToDebt, withdrawAmount, deposited, borrowed, currentLedgerInstant, isClosingPosition, selectedTokenIsCollToken, selectedTokenMint, obligation, flashLoanFee, slippagePct) {
    ...
    (0, swap_calcs_1.assertPositiveFiniteDecimal)('withdrawLeverageCalcs: priceCollToDebt', priceCollToDebt);
    ...
    const { earlyRepayPenaltyAmount, repayFundingAmount } = leverageEarlyRepayPenalty(obligation, debtReserve, repayAmount, currentLedgerInstant);
    // 6. Get swap ixs
 ...
 */
function calcCollFlashLegLamports(params) {
    ...
    const flashFeeLamports = (0, repay_with_collateral_calcs_1.calcFlashLoanFees)({
        reserve: collReserve,
        referralFeeBps: 0,
        hasReferral: false,
 ...
 * ledger instant.
 */
function leverageEarlyRepayPenalty(obligation, debtReserve, repayAmountTokens, currentLedgerInstant) {
    ...
    if (!debtReserve.getKind().isFixedRate()) {
        return { earlyRepayPenaltyAmount: new decimal_js_1.default(0), repayFundingAmount: repayAmountTokens };
    }
...
function adjustDepositLeverageCalcs(debtReserve, adjustDepositPosition, adjustBorrowPosition, priceDebtToColl, flashLoanFee, slippagePct) {
    (0, swap_calcs_1.assertPositiveFiniteDecimal)('adjustDepositLeverageCalcs: priceDebtToColl', priceDebtToColl);
    ...
    return {
        ...
        earlyRepayPenaltyAmount: new decimal_js_1.default(0),
        repayFundingAmount: new decimal_js_1.default(0),
    };
}
...
obligation, debtReserve, currentLedgerInstant) {
    ...
    const absRepay = decimal_js_1.default.abs(adjustBorrowPosition);
    const { earlyRepayPenaltyAmount, repayFundingAmount } = obligation && debtReserve && currentLedgerInstant !== undefined
        ? leverageEarlyRepayPenalty(obligation, debtReserve, absRepay, currentLedgerInstant)
        : { earlyRepayPenaltyAmount: new decimal_js_1.default(0), repayFundingAmount: absRepay };
    const fundingScale = absRepay.gt(0) ? repayFundingAmount.div(absRepay) : new decimal_js_1.default(1);
 ...
 */
const depositLeverageCalcsDebtFlash = (props) => {
    const { depositAmount, depositTokenIsCollToken, depositTokenIsSol, priceDebtToColl, targetLeverage, slippagePct, flashLoanFee, } = props;
    const slippage = slippagePct.div('100');
    const initDepositInSol = depositTokenIsSol ? depositAmount : new decimal_js_1.default(0);
    (0, swap_calcs_1.assertPositiveFiniteDecimal)('depositLeverageCalcsDebtFlash: priceDebtToColl', priceDebtToColl);
    if (depositTokenIsCollToken) {
        ...
        const denominator = priceDebtToColl.mul(targetLeverage.mul(flashFeeFactor).sub(targetLeverage.sub('1').div(slippageFactor)));
        ...
        if (denominator.isZero()) {
            throw new Error('depositLeverageCalcsDebtFlash: denominator is zero — check targetLeverage, slippage, and flashLoanFee');
        }
        const flashBorrowDebt = depositAmount.mul(targetLeverage.sub('1')).div(denominator);
...
exports.depositLeverageCalcsDebtFlash = depositLeverageCalcsDebtFlash;
/**
 ...
 */
function withdrawLeverageCalcsCollFlash(market, collReserve, debtReserve, priceCollToDebt, withdrawAmount, deposited, borrowed, currentLedgerInstant, isClosingPosition, selectedTokenIsCollToken, selectedTokenMint, obligation, flashLoanFee, slippagePct) {
    ...
    (0, swap_calcs_1.assertPositiveFiniteDecimal)('withdrawLeverageCalcsCollFlash: priceCollToDebt', priceCollToDebt);
    const swapAmountIfWithdrawingColl = repayFundingAmount
        .mul(new decimal_js_1.default(1).add(slippagePct.div(100)))
        .div(priceCollToDebt);
    const swapAmountIfWithdrawingDebt = withdrawAmountCalculated;
 ...
 * Flow: flash borrow debt -> swap debt->coll -> deposit coll -> borrow debt -> flash repay debt
 */
function adjustDepositLeverageCalcsDebtFlash(debtReserve, adjustDepositPosition, adjustBorrowPosition, priceDebtToColl, flashLoanFee, slippagePct) {
    ...
    (0, swap_calcs_1.assertPositiveFiniteDecimal)('adjustDepositLeverageCalcsDebtFlash: priceDebtToColl', priceDebtToColl);
    const flashBorrowDebt = adjustDepositPosition
        .mul(new decimal_js_1.default(1).add(slippagePct.div(100)))
        .div(priceDebtToColl)
        .toDecimalPlaces(debtReserve.stats.decimals, decimal_js_1.default.ROUND_UP);
    // We borrow enough debt from klend to repay flash loan + fee
    const debtTokenToBorrow = flashBorrowDebt.mul(new decimal_js_1.default(1).add(flashLoanFee));
    return {
        adjustDepositPosition,
        adjustBorrowPosition,
        flashBorrowInDebtToken: flashBorrowDebt,
 ...
 * Flow: flash borrow coll -> swap coll->debt -> repay debt -> withdraw coll -> flash repay coll
 */
function adjustWithdrawLeverageCalcsCollFlash(adjustDepositPosition, adjustBorrowPosition, priceCollToDebt, flashLoanFee, slippagePct, 
// Optional for pure calculation callers. Production builders and flash-borrow selectors provide the obligation,
// reserve, and ledger instant so the fixed-term early-repay penalty is folded into the sizing.
obligation, debtReserve, currentLedgerInstant) {
    const absDebtRepay = decimal_js_1.default.abs(adjustBorrowPosition);
    // Fixed-term debt charges an early-repay penalty on top of the repay; the coll→debt swap must produce
    ...
    const { earlyRepayPenaltyAmount, repayFundingAmount } = obligation && debtReserve && currentLedgerInstant !== undefined
        ...
        : { earlyRepayPenaltyAmount: new decimal_js_1.default(0), repayFundingAmount: absDebtRepay };
    // Flash borrow coll to swap for debt repayment (incl. penalty)
    // collSwapIn * priceCollToDebt / (1 + slippage) >= repayFundingAmount
    (0, swap_calcs_1.assertPositiveFiniteDecimal)('adjustWithdrawLeverageCalcsCollFlash: priceCollToDebt', priceCollToDebt);
    const collTokenSwapIn = repayFundingAmount.mul(new decimal_js_1.default(1).add(slippagePct.div(100))).div(priceCollToDebt);
    const debtTokenExpectedSwapOut = collTokenSwapIn.mul(priceCollToDebt).div(new decimal_js_1.default(1).add(slippagePct.div(100)));
