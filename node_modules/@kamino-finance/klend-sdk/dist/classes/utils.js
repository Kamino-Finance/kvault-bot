...
exports.positiveOrZero = exports.isEmptyObject = exports.valueOrZero = exports.isSolMint = exports.noop = exports.parseTokenSymbol = exports.getBorrowRate = exports.interpolate = exports.estimateCurrentScore = exports.calculateNewScore = void 0;
...
const calculateNewScore = (rewardStat, pool, rewardRate, endSlot, startSlot) => {
    ...
    const newRewardsPerShare = !totalBalanceVal.isZero()
        ? oldRewardsPerShare.plus(new decimal_js_1.default(endSlot)
            ...
            .div(totalBalanceVal)
            .div(new decimal_js_1.default(constants_1.SLOTS_PER_YEAR)))
        : new decimal_js_1.default(0);
    return oldScore.plus(newRewardsPerShare.times(oldBalance).minus(oldDebt));
};
exports.calculateNewScore = calculateNewScore;
const estimateCurrentScore = (rewardStat, rewardScore, currentSlot) => {
    const { lastSlot, rewardRates } = rewardStat;
    const { rewardRate } = getLatestRewardRate(rewardRates, currentSlot);
    const currentScore = (0, exports.calculateNewScore)(rewardStat, rewardScore, rewardRate, currentSlot, lastSlot);
    return currentScore;
};
...
const isSolMint = (mint) => {
    return consts_1.WRAPPED_SOL_MINT === mint;
};
...
function calculateAPYFromAPR(apr) {
    const apy = new decimal_js_1.default(1).plus(new decimal_js_1.default(apr).dividedBy(constants_1.SLOTS_PER_YEAR)).toNumber() ** constants_1.SLOTS_PER_YEAR - 1;
    return apy;
}
...
function calculateAPRFromAPY(apy) {
    return new decimal_js_1.default(apy)
        .plus(1)
        .pow(1 / constants_1.SLOTS_PER_YEAR)
        .minus(1)
        .times(constants_1.SLOTS_PER_YEAR);
}
