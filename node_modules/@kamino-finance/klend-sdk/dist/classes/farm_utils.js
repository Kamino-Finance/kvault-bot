...
exports.FARMS_GLOBAL_CONFIG_DEVNET = exports.FARMS_GLOBAL_CONFIG_MAINNET = void 0;
exports.fetchFarmStateOrNull = fetchFarmStateOrNull;
...
exports.getValidatedFarmRewardTimeUnit = getValidatedFarmRewardTimeUnit;
exports.getUpdateFarmPendingAdminIx = getUpdateFarmPendingAdminIx;
...
async function fetchFarmStateOrNull(rpc, farmAddress) {
    const farmState = await (0, farms_sdk_1.fetchMaybeFarmState)(rpc, farmAddress);
...
async function setVaultIdForFarmIx(rpc, farmAdmin, farm, vault, farmsProgramId) {
    const farmClient = new farms_sdk_1.Farms(rpc, farmsProgramId);
    return farmClient.updateFarmConfigIx(farmAdmin, farm, utils_1.DEFAULT_PUBLIC_KEY, farms_sdk_1.FarmConfigOption.UpdateVaultId, vault);
}
/**
...
function getRewardPerTimeUnitSecond(reward, farmTotalStakeLamports, currentTimestampSec) {
    const now = new decimal_1.default(currentTimestampSec);
    ...
    const constantRewardStakeAdjustment = reward.rewardType === farms_sdk_1.RewardType.Constant ? farmTotalStakeLamports : new decimal_1.default(1);
 ...
 */
async function getUserPendingRewardsInFarm(rpc, userStateAddress, farm, currentLedgerInstant, farmsProgramId) {
    ...
    const userState = (0, farms_sdk_1.decodeUserState)(userStateAccountInfo).data;
...
async function getValidatedFarmRewardTimeUnit(farm, farmState, currentLedgerInstant) {
    let timeUnitName;
    switch (farmState.timeUnit) {
        case farms_sdk_1.TimeUnit.Seconds:
            timeUnitName = 'block time';
            break;
        case farms_sdk_1.TimeUnit.Slots:
            timeUnitName = 'slot';
            break;
        default:
            throw new Error(`Farm ${farm} has unsupported time unit ${farmState.timeUnit}`);
    }
    const currentTimeUnit = await (0, farms_sdk_1.getCurrentTimeUnit)(farmState, currentLedgerInstant.slot, currentLedgerInstant.blockTime);
    ...
    if (currentTimeUnit.lt(latestRewardIssuanceTimeUnit.toString())) {
        throw new Error(`Ledger instant ${timeUnitName} ${currentTimeUnit.toString()} predates farm ${farm} reward state ${timeUnitName} ${latestRewardIssuanceTimeUnit.toString()}. Fetch a newer ledger instant and retry.`);
    }
 ...
 */
async function getUpdateFarmPendingAdminIx(rpc, signer, farm, newPendingAdmin, farmsProgramId) {
    const farmClient = new farms_sdk_1.Farms(rpc, farmsProgramId);
    return await farmClient.updateFarmConfigIx(signer, farm, utils_1.DEFAULT_PUBLIC_KEY, farms_sdk_1.FarmConfigOption.UpdatePendingFarmAdmin, newPendingAdmin, undefined, undefined, true);
}
