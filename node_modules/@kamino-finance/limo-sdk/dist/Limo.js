...
exports.limoId = (0, kit_1.address)("LiMoM9rMhrdYrfzUCxQppvxCSG1FcrUK9G8uLq4A1GF");
exports.WRAPPED_SOL_MINT = (0, kit_1.address)("So11111111111111111111111111111111111111112");
exports.ORDER_RENT_EXEMPTION_LAMPORTS = BigInt(3841920);
...
class LimoClient {
    ...
    globalConfigState;
    _globalConfig;
    constructor(connection, subscription, globalConfig, globalConfigState) {
        this._connection = connection;
        this._subscription = subscription;
        this._globalConfig = globalConfig ?? utils_1.DEFAULT_ADDRESS;
        this.programAddress = exports.limoId;
    ...
    async refreshGlobalConfigState() {
        ...
        const globalConfigState = await accounts_1.GlobalConfig.fetch(this._connection, this._globalConfig);
    ...
    async getOrderState(orderAddress) {
        const order = await accounts_1.Order.fetch(this._connection, orderAddress, this.programAddress);
        if (!order) {
    ...
    async getAllOrdersStateAndAddressWithFilters(filters, globalConfigOverride, filterByGlobalConfig = true) {
        ...
        filters.push({
            dataSize: BigInt(accounts_1.Order.layout.span + 8),
        });
        ...
        return orderProgramAccounts.map((orderProgramAccount) => {
            ...
            const order = accounts_1.Order.decode(buffer);
            if (!order) {
     ...
     */
    async listenToMakerOrders(maker, callbackOnChange) {
        const filters = [
            {
                memcmp: {
                    bytes: maker.toString(),
        ...
        const abortController = await this.listenToOrdersChangeWithFilters(filters, callbackOnChange);
     ...
     */
    async listenToOrderChangeForBaseAndQuote(baseTokenMint, quoteTokenMint, callbackOnChangeSellOrders, callbackOnChangeBuyOrders) {
        const buyFilters = [
            {
                memcmp: {
                    bytes: baseTokenMint.toString(),
                    encoding: "base58",
                    offset: BigInt(72),
                },
            },
        ...
        const abortControllerSellOrders = await this.listenToOrdersChangeWithFilters(sellFilters, callbackOnChangeSellOrders);
        const abortControllerBuyOrders = await this.listenToOrdersChangeWithFilters(buyFilters, callbackOnChangeBuyOrders);
     ...
     * @returns { subscriptionIdSellOrders, subscriptionIdBuyOrders } - the subscription id for the two listeners (both should be closed when subscription no longer needed)
     */
    async listenToOrderFillChangeForBaseAndQuote(baseTokenMint, quoteTokenMint, callbackOnChangeSellOrders, callbackOnChangeBuyOrders) {
        const buyFilters = [
            {
                memcmp: {
                    bytes: baseTokenMint.toString(),
        ...
        const abortControllerSellOrders = await this.listenToOrdersChangeWithFilters(sellFilters, callbackOnChangeSellOrdersFilledOrdersOnly);
        const abortControllerBuyOrders = await this.listenToOrdersChangeWithFilters(buyFilters, callbackOnChangeBuyOrdersFilledOrdersOnly);
        return { abortControllerSellOrders, abortControllerBuyOrders };
    }
    /**
     ...
     * @returns subscriptionId - a number of the subscription id, to be used to stop the listener
     */
    async listenToOrdersChangeWithFilters(filters, callbackOnChange) {
        filters.push({
            memcmp: {
                bytes: this._globalConfig.toString(),
                encoding: "base58",
                offset: BigInt(8),
            },
        });
        ...
        const callbackOnChangeWtihDecoding = async (keyedAccountInfo, context) => {
            ...
            const order = accounts_1.Order.decode(Buffer.from(base64Data, encoding));
            if (!order) {
                throw Error("Could not parse obligation.");
            }
            callbackOnChange({
     ...
     */
    stopListeningToOrdersChange(abortController) {
    ...
    async createGlobalConfigIxs(admin, globalConfig) {
        let ixs = [];
        const globalConfigSize = accounts_1.GlobalConfig.layout.span + 8;
        const lamports = await this._connection
            .getMinimumBalanceForRentExemption(BigInt(globalConfigSize))
            .send();
        ixs.push((0, utils_1.createKeypairRentExemptIxSync)(admin, globalConfig, globalConfigSize, lamports.valueOf(), this.programAddress));
        const pdaAuthority = await (0, utils_1.getPdaAuthority)(this.programAddress, globalConfig.address);
        ixs.push(limoOperations.initializeGlobalConfig(admin, globalConfig.address, pdaAuthority, this.programAddress));
        return ixs;
    }
    /**
    ...
    async initializeVaultIx(user, mint, mintTokenProgramId) {
        ...
        return limoOperations.initializeVault(user, this._globalConfig, mint, this.programAddress, mintProgramId);
    }
    /**
    ...
    async initializeVault(user, mint, mode = "execute", mintTokenProgramId) {
        ...
        const vault = (0, utils_1.getTokenVaultPDA)(this.programAddress, this._globalConfig, mint);
    ...
    async createOrderGenericIx(user, inputMint, outputMint, inputAmountLamports, outputAmountLamports, inputMintProgramId, outputMintProgramId, globalConfigOverride, wrapUnwrapSol = true, withInitVault) {
        ...
        if (withInitVault === undefined) {
            try {
                const vaultPda = await (0, utils_1.getTokenVaultPDA)(this.programAddress, this._globalConfig, inputMint);
                const accountExists = await (0, utils_1.checkIfAccountExists)(this._connection, vaultPda);
        ...
        const order = await (0, signers_1.generateKeyPairSigner)();
        const orderParams = {
            side: "bid",
        ...
        const ixs = [];
        if (initVault) {
            ixs.push(await limoOperations.initializeVault(user, this._globalConfig, inputMint, this.programAddress, inputMintProgramId));
        }
        ixs.push((0, utils_1.createKeypairRentExemptIxSync)(user, order, accounts_1.Order.layout.span + 8, exports.ORDER_RENT_EXEMPTION_LAMPORTS, this.programAddress));
        const baseTokenMintProgramId = inputMintProgramId;
        const quoteTokenMintProgramId = outputMintProgramId;
        let closeWsolAtaIxs = [];
        if (inputMint === exports.WRAPPED_SOL_MINT && wrapUnwrapSol) {
            const { createIxs, fillIxs, closeIx } = await this.getInitIfNeededWSOLCreateAndCloseIxs(user, user, inputAmountLamports);
            ixs.push(...createIxs, ...fillIxs);
            closeWsolAtaIxs = closeIx;
        }
        ixs.push(await limoOperations.createOrder(user, globalConfigOverride ? globalConfigOverride : this._globalConfig, order.address, orderParams, this.programAddress, baseTokenMintProgramId, quoteTokenMintProgramId));
     ...
     * @throws error if mint decimals not found for mint
     */
    async createOrderGenericWithParamsIx(args) {
        const { user, inputMint, outputMint, inputAmountLamports, outputAmountLamports, inputMintProgramId, outputMintProgramId, permissionless, counterparty, globalConfigOverride, wrapUnwrapSol = args.wrapUnwrapSol !== undefined
            ? args.wrapUnwrapSol
            : true, withInitVault, } = args;
        const [ixs, order] = await this.createOrderGenericIx(user, inputMint, outputMint, inputAmountLamports, outputAmountLamports, inputMintProgramId, outputMintProgramId, globalConfigOverride, wrapUnwrapSol, withInitVault);
        if (permissionless) {
            const updateOrderPermissionlessIx = this.updateOrderIx("UpdatePermissionless", true, user, globalConfigOverride ? globalConfigOverride : this._globalConfig, order.address);
            ixs.push(...updateOrderPermissionlessIx);
        }
        if (counterparty) {
            const updateOrderCounterpartyIx = this.updateOrderIx("UpdateCounterparty", counterparty, user, globalConfigOverride ? globalConfigOverride : this._globalConfig, order.address);
            ixs.push(...updateOrderCounterpartyIx);
        }
        return [ixs, order];
    }
    async createOrderGenericWithParams(args, user, mode = "execute") {
    ...
    async createOrderGeneric(user, inputMint, outputMint, inputAmountLamports, outputAmountLamports, mode = "execute", inputMintProgramId, outputMintProgramId, globalConfigOverride) {
        const [ixs, order] = await this.createOrderGenericIx(user, inputMint, outputMint, inputAmountLamports, outputAmountLamports, inputMintProgramId, outputMintProgramId, globalConfigOverride);
        const log = "Create Order: " + order.address;
        const sig = await this.processTxn(user, ixs, mode, log, [order]);
        return [sig, order];
    }
    ...
    async placeBid(user, quoteTokenMint, baseTokenMint, baseUiAmount, price, mode = "execute", inputMintProgramId, outputMintProgramId, mintDecimals, globalConfigOverride) {
        const [ixs, order] = await this.placeBidIxs(user, quoteTokenMint, baseTokenMint, baseUiAmount, price, inputMintProgramId, outputMintProgramId, mintDecimals, globalConfigOverride);
    ...
    async placeAsk(user, quoteTokenMint, baseTokenMint, quoteUiAmount, price, mode = "execute", inputMintProgramId, outputMintProgramId, mintDecimals, globalConfigOverride) {
        const [ixs, order] = await this.placeAskIxs(user, quoteTokenMint, baseTokenMint, quoteUiAmount, price, inputMintProgramId, outputMintProgramId, mintDecimals, globalConfigOverride);
     ...
     */
    async takeOrderIx(taker, order, inputAmountLamports, minOutputAmountLamports, expressRelayProgramId, permissionlessTipLamports, permissionless, wrapUnwrapSol = true) {
        let ixs = [];
        let closeWsolAtaIxs = [];
        let takerInputAta;
        if (order.state.inputMint === exports.WRAPPED_SOL_MINT) {
            const { createIxs, fillIxs: _fill, closeIx, ata, } = await this.getInitIfNeededWSOLCreateAndCloseIxs(taker, taker, new bn_js_1.default(0));
            takerInputAta = ata;
            if (wrapUnwrapSol) {
                ixs.push(...createIxs);
            ...
            const { ata, createAtaIx: createTakerInputAta } = await (0, utils_1.createAtaIdempotent)(taker.address, taker, order.state.inputMint, order.state.inputMintProgramId);
            takerInputAta = ata;
            ixs.push(createTakerInputAta);
        }
        let takerOutputAta;
        if (order.state.outputMint === exports.WRAPPED_SOL_MINT) {
            ...
            const { createIxs, fillIxs, closeIx, ata } = await this.getInitIfNeededWSOLCreateAndCloseIxs(taker, taker, outputExpectedOutForInputAmount);
            takerOutputAta = ata;
            if (wrapUnwrapSol) {
                ixs.push(...createIxs, ...fillIxs);
                closeWsolAtaIxs.push(...closeIx);
            ...
            const { ata, createAtaIx: createTakerOutputAta } = await (0, utils_1.createAtaIdempotent)(taker.address, taker, order.state.outputMint, order.state.outputMintProgramId);
            takerOutputAta = ata;
        ...
        if (order.state.outputMint === exports.WRAPPED_SOL_MINT) {
            ...
            intermediaryOutputTokenAccount = await (0, utils_1.getIntermediaryTokenAccountPDA)(this.programAddress, order.address);
        }
        else {
            // create maker ata
            const { ata, createAtaIx } = await (0, utils_1.createAtaIdempotent)(order.state.maker, taker, order.state.outputMint, order.state.outputMintProgramId);
            makerOutputAta = ata;
            ixs.push(createAtaIx);
            intermediaryOutputTokenAccount = undefined;
        }
        ixs.push(await limoOperations.takeOrder({
            taker,
            maker: order.state.maker,
    ...
    async permissionlessTakeOrder(crank, order, inputAmountLamports, outputAmountLamports, expressRelayProgramId, mode, permissionlessTipLamports, mintDecimals) {
        ...
        const ixs = await this.takeOrderIx(crank, order, inputAmountLamports, outputAmountLamports, expressRelayProgramId, permissionlessTipLamports, true);
        const outputExpectedOutForInputAmount = (0, utils_1.divCeil)(order.state.expectedOutputAmount.mul(inputAmountLamports), order.state.initialInputAmount);
     ...
     */
    async flashTakeOrderIxs(taker, order, inputAmountLamports, minOutputAmountLamports, expressRelayProgramId, permissionlessTipLamports, permissionless, wrapUnwrapSol = true) {
        let createAtaIxs = [];
        let closeWsolAtaIxs = [];
        let takerInputAta;
        if (order.state.inputMint === exports.WRAPPED_SOL_MINT) {
        ...
        if (order.state.outputMint === exports.WRAPPED_SOL_MINT) {
            const outputExpectedOutForInputAmount = (0, utils_1.divCeil)(order.state.expectedOutputAmount.mul(inputAmountLamports), order.state.initialInputAmount);
            const { createIxs, fillIxs: _fillIxs, closeIx, ata, } = await this.getInitIfNeededWSOLCreateAndCloseIxs(taker, taker, outputExpectedOutForInputAmount);
            takerOutputAta = ata;
            if (wrapUnwrapSol) {
                createAtaIxs.push(...createIxs);
                closeWsolAtaIxs.push(...closeIx);
        ...
        const { startIx: startFlashIx, endIx: endFlashIx } = await limoOperations.flashTakeOrder({
            taker,
            maker: order.state.maker,
            globalConfig: order.state.globalConfig,
            inputMint: order.state.inputMint,
    ...
    async permissionlessFlashTakeOrder(crank, order, inputAmountLamports, outputAmountLamports, expressRelayProgramId, mode, swapIxs, permissionlessTipLamports, extraSigners, mintDecimals) {
        ...
        outputMintDecimals = outputMintDecimals
            ? outputMintDecimals
            : await (0, utils_1.getMintDecimals)(this._connection, order.state.outputMint);
        const { createAtaIxs, startFlashIx, endFlashIx, closeWsolAtaIxs } = await this.flashTakeOrderIxs(crank, order, inputAmountLamports, outputAmountLamports, expressRelayProgramId, permissionlessTipLamports, true);
        const outputExpectedOutForInputAmount = (0, utils_1.divCeil)(order.state.expectedOutputAmount.mul(inputAmountLamports), order.state.initialInputAmount);
        const log = "Taker Order: " +
            order.address +
            " selling " +
     ...
     */
    async logUserSwapBalancesIxs(args) {
        const { user, inputMint, outputMint, inputTa, outputTa, swapProgarmId, simulatedSwapAmountOut, simulatedTs, minimumAmountOut, swapAmountIn, simulatedAmountOutNextBest, aggregatorId, nextBestAggregatorId, pdaReferrer = args.pdaReferrer ?? this.programAddress, voteAccount, } = args;
        const logIxStart = (0, instructions_1.logUserSwapBalancesStart)({
            baseAccounts: {
                ...
                inputTa,
                outputTa,
                pdaReferrer: (0, utils_1.asOption)(pdaReferrer),
                swapProgramId: swapProgarmId,
            },
            userSwapBalanceState: await (0, utils_1.getUserSwapBalanceStatePDA)(user.address, this.programAddress),
            eventAuthority: await (0, utils_1.getEventAuthorityPDA)(this.programAddress),
        ...
        const logIxEnd = (0, instructions_1.logUserSwapBalancesEnd)({
     ...
     * @returns the log ixs - to be used once at the beginning and once at the end
     */
    async logUserSwapBalances(user, inputMint, outputMint, inputMintProgramId, outputMintProgramId, setupIxs = [], mockSwapIxs = [], mockSwapSigners = [], swapProgarmId, voteAccount) {
        const [inputTa] = await (0, token_1.findAssociatedTokenPda)({
            mint: inputMint,
            owner: user.address,
            tokenProgram: inputMintProgramId,
        });
        const [outputTa] = await (0, token_1.findAssociatedTokenPda)({
            mint: outputMint,
            owner: user.address,
        ...
        const { beforeSwapIx, afterSwapIx } = await this.logUserSwapBalancesIxs({
            ...
            outputTa,
            swapProgarmId,
            simulatedSwapAmountOut: new bn_js_1.default(0),
            simulatedTs: new bn_js_1.default(0),
            minimumAmountOut: new bn_js_1.default(0),
            swapAmountIn: new bn_js_1.default(0),
            simulatedAmountOutNextBest: new bn_js_1.default(0),
            aggregatorId: 0,
     ...
     * @returns the log ixs - to be used once at the beginning and once at the end
     */
    async assertUserSwapBalancesIxs(args) {
        const { user, inputMint, outputMint, inputMintProgramId, outputMintProgramId, maxInputAmountChange, minOutputAmountChange, inputTa, outputTa, } = args;
        let inputMintTa;
        if (inputTa) {
            inputMintTa = inputTa;
        }
        else if (inputMint && inputMintProgramId) {
            [inputMintTa] = await (0, token_1.findAssociatedTokenPda)({
                mint: inputMint,
                owner: user.address,
                tokenProgram: inputMintProgramId,
            });
            ...
            throw new Error("Input mint and program ID must be provided if inputTa is not given");
        }
        let outputMintTa;
        if (outputTa) {
            ...
            [outputMintTa] = await (0, token_1.findAssociatedTokenPda)({
                mint: outputMint,
                owner: user.address,
                tokenProgram: outputMintProgramId,
            });
            ...
            throw new Error("Output mint and program ID must be provided if outputTa is not given");
        }
        const userSwapBalanceState = await (0, utils_1.getUserSwapBalanceAssertStatePDA)(user.address, this.programAddress);
        const logIxStart = (0, instructions_1.assertUserSwapBalancesStart)({
            maker: user,
            inputTa: inputMintTa,
        ...
        const logIxEnd = (0, instructions_1.assertUserSwapBalancesEnd)({
     ...
     * @returns the log ixs - to be used once at the beginning and once at the end
     */
    async assertUserSwapBalances(user, inputMint, outputMint, inputMintProgramId, outputMintProgramId, maxInputAmountChange, minOutputAmountChange, setupIxs = [], mockSwapIxs = [], mockSwapSigners = []) {
     ...
     */
    async getCloseAndClaimTipsForFilledOrdersTxsIxs(maker, orders, batchSize = exports.MAX_CLOSE_ORDER_AND_CLAIM_TIP_ORDERS_IN_TX) {
        let ixsArrays = [];
        ixsArrays.push([]);
        for (const order of orders) {
            if (order.state.status === 1) {
                ...
                const ixs = await this.closeOrderAndClaimTipIx(maker, orderStateAndAddress);
     ...
     */
    async closeOrderAndClaimTipIx(maker, order, wrapUnwrapSol = true) {
        let ixs = [];
        let closeWsolAtaIxs = [];
        let makerInputAta;
        if (order.state.inputMint === exports.WRAPPED_SOL_MINT) {
            const { createIxs, fillIxs: _fill, closeIx, ata, } = await this.getInitIfNeededWSOLCreateAndCloseIxs(maker, maker, new bn_js_1.default(0));
            makerInputAta = ata;
            if (wrapUnwrapSol) {
                ixs.push(...createIxs);
                closeWsolAtaIxs.push(...closeIx);
            ...
            const { ata, createAtaIx: createMakerInputAta } = await (0, utils_1.createAtaIdempotent)(maker.address, maker, order.state.inputMint, order.state.inputMintProgramId);
            makerInputAta = ata;
            ixs.push(createMakerInputAta);
        }
        ixs.push(await limoOperations.closeOrderAndClaimTip({
            maker: maker,
            globalConfig: order.state.globalConfig,
            inputMint: order.state.inputMint,
            outputMint: order.state.outputMint,
    ...
    async closeOrderAndClaimTip(maker, order, mode, mintDecimals) {
        ...
        if (mintDecimals) {
            inputMintDecimals = mintDecimals.get(order.state.inputMint);
            tipMintDecimals = mintDecimals.get(exports.WRAPPED_SOL_MINT);
        }
        inputMintDecimals = inputMintDecimals
            ? inputMintDecimals
            : await (0, utils_1.getMintDecimals)(this._connection, order.state.inputMint);
        tipMintDecimals = tipMintDecimals
            ? tipMintDecimals
            : await (0, utils_1.getMintDecimals)(this._connection, exports.WRAPPED_SOL_MINT);
    ...
    async updateOrderGenericIx(order, user, inputMint, outputMint, inputAmountLamports, outputAmountLamports, inputMintProgramId, outputMintProgramId, globalConfigOverride, wrapUnwrapSol = true) {
        let closeOrderIx = await this.closeOrderAndClaimTipIx(user, order, wrapUnwrapSol);
    ...
    async updateOrderGeneric(order, user, inputMint, outputMint, inputAmountLamports, outputAmountLamports, inputMintProgramId, outputMintProgramId, mode = "execute", globalConfigOverride, wrapUnwrapSol = true) {
        const [ixs, orderKp] = await this.updateOrderGenericIx(order, user, inputMint, outputMint, inputAmountLamports, outputAmountLamports, inputMintProgramId, outputMintProgramId, globalConfigOverride, wrapUnwrapSol);
    ...
    updateOrderIx(mode, value, maker, globalConfig, order) {
        const ixs = [];
        ixs.push(limoOperations.updateOrder(types_1.UpdateOrderMode.fromDecoded({ [mode]: "" }), value, maker, globalConfig, order, this.programAddress));
        return ixs;
    }
    updateOrder(maker, updateMode, value, order, mode) {
        const ixs = this.updateOrderIx(updateMode, value, maker, this._globalConfig, order);
     ...
     */
    async updateGlobalConfigIx(admin, mode, value, globalConfigOverride) {
        const ixs = [];
        ixs.push(limoOperations.updateGlobalConfigIx(admin, globalConfigOverride ? globalConfigOverride : this._globalConfig, types_1.UpdateGlobalConfigMode.fromDecoded({ [mode]: "" }), value, this.programAddress));
        return ixs;
    }
    /**
    ...
    async updateGlobalConfig(admin, updateMode, value, mode, globalConfigOverride) {
        ...
        const ixs = await this.updateGlobalConfigIx(admin, updateMode, value, globalConfigOverride);
     ...
     */
    async updateGlobalConfigAdminIx(admin) {
        const globalConfigState = await this.getGlobalConfigState();
        if (!globalConfigState) {
            throw new Error("Global config not found");
        }
        const ix = limoOperations.updateGlobalConfigAdminIx(admin, this._globalConfig, globalConfigState, this.programAddress);
        return ix;
    }
    /**
     ...
     * @returns the transaction signature
     */
    async updateGlobalConfigAdmin(admin, mode) {
     ...
     */
    async withdrawHostTipIx(admin, globalConfigOverride) {
        let ixs = [];
        ixs.push(await (0, utils_1.withdrawHostTipIx)({
            admin,
     ...
     * @returns the transaction signature
     */
    async executeTransaction(instructions, signer, extraSigners = []) {
        const { value: latestBlockhash } = await this._connection
            .getLatestBlockhash()
            .send();
        const transactionMessage = (0, kit_1.pipe)((0, kit_1.createTransactionMessage)({ version: 0 }), // Create transaction message
        (tx) => (0, kit_1.setTransactionMessageFeePayerSigner)(signer, tx), (tx) => (0, kit_1.setTransactionMessageLifetimeUsingBlockhash)(latestBlockhash, tx), (tx) => (0, kit_1.appendTransactionMessageInstructions)(instructions, tx));
        const transactionsMessageWithSigners = extraSigners
            ? (0, signers_1.addSignersToTransactionMessage)(extraSigners, transactionMessage)
            : transactionMessage;
        const signedTransaction = await (0, kit_1.signTransactionMessageWithSigners)(transactionsMessageWithSigners);
        await (0, kit_1.sendAndConfirmTransactionFactory)({
     ...
     * @returns the transaction signature
     */
    async processTxn(admin, instructions, mode, debugMessage, extraSigners, computeUnits = 200_000, priorityFeeLamports = 10000) {
        if (mode === "multisig" || mode === "simulate") {
            const { value: latestBlockhash } = await this._connection
                .getLatestBlockhash()
                .send();
            const transactionMessage = (0, kit_1.pipe)((0, kit_1.createTransactionMessage)({ version: 0 }), // Create transaction message
            (tx) => (0, kit_1.setTransactionMessageFeePayerSigner)(admin, tx), (tx) => (0, kit_1.setTransactionMessageLifetimeUsingBlockhash)(latestBlockhash, tx), (tx) => (0, kit_1.appendTransactionMessageInstructions)(instructions, tx));
            const transactionsMessageWithSigners = extraSigners
                ? (0, signers_1.addSignersToTransactionMessage)(extraSigners, transactionMessage)
                : transactionMessage;
            const signedTransaction = await (0, kit_1.signTransactionMessageWithSigners)(transactionsMessageWithSigners);
            ...
            if (mode === "simulate") {
                await (0, utils_1.printSimulateTx)(this._connection, signedTransaction);
            }
            else {
                // if simulate is false (multisig is true)
                await (0, utils_1.printMultisigTx)(signedTransaction);
            }
            return "";
        }
            ...
            const sig = await this.executeTransaction([...priorityFeeIxs, ...instructions], admin, extraSigners);
     ...
     */
    async getInitIfNeededWSOLCreateAndCloseIxs(owner, payer, amountToDepositLamports) {
        const createIxs = [];
        const { ata, createAtaIx } = await (0, utils_1.createAtaIdempotent)(owner.address, payer, exports.WRAPPED_SOL_MINT);
        ...
        if (payer === owner) {
            closeWsolAtaIxn.push((0, token_1.getCloseAccountInstruction)({
                account: ata,
    ...
    getDepositWsolIxns(owner, ata, amountLamports) {
        ...
        ixns.push((0, system_1.getTransferSolInstruction)({
            source: owner,
            destination: ata,
