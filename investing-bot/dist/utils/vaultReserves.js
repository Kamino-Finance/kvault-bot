...
export const VAULT_RESERVES_MAX_UNIQUE_RESERVES_PER_BATCH = getEnvOrDefaultNum('VAULT_RESERVES_MAX_UNIQUE_RESERVES_PER_BATCH', DEFAULT_MAX_UNIQUE_RESERVES_PER_BATCH);
// Keep this sequential: the SDK reserve loader fans out account/oracle reads internally.
export async function loadVaultsReservesInBatches(kaminoManager, vaultStates, label, heartbeat, maxUniqueReservesPerBatch = VAULT_RESERVES_MAX_UNIQUE_RESERVES_PER_BATCH) {
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex += 1) {
        ...
        const batchReserves = await withTimeout(kaminoManager.loadVaultsReserves(batch), RPC_REQUEST_TIMEOUT_MS, `${label} batch ${batchIndex + 1}/${batches.length}`);
...
function buildVaultStateReserveBatches(kaminoManager, vaultStates, maxUniqueReservesPerBatch) {
    const batches = [];
    let currentBatch = [];
    let currentReserveKeys = new Set();
...
function uniqueReserveKeys(reserves) {
    return [...new Set(reserves.map((reserve) => reserve.toString()))];
}
//# sourceMappingURL=vaultReserves.js.map