 ...
 */
export class MarketUtilizationTrigger {
    name = 'market-utilization';
    safeUtilization;
    /** @param safeUtilization highest utilization still scored fully safe; must be in [0, 1). */
    constructor(safeUtilization = DEFAULT_SAFE_UTILIZATION) {
        if (!Number.isFinite(safeUtilization) || safeUtilization < 0 || safeUtilization >= FULL_UTILIZATION) {
            throw new Error(`[danger-detection] ${MarketUtilizationTrigger.name} safeUtilization must be in [0, ${FULL_UTILIZATION}), got ${safeUtilization}`);
        }
    ...
    async check(_rpc, reserveAddress, reserve, context) {
        try {
            ...
            const safetyScore = Math.max(0, Math.min(1.0, (FULL_UTILIZATION - utilization) / (FULL_UTILIZATION - this.safeUtilization)));
            const details = `utilization ${(utilization * 100).toFixed(2)}% vs safe threshold ${(this.safeUtilization * 100).toFixed(2)}%`;
            if (safetyScore < 1.0) {
                logger.warn(`[danger-detection] market utilization for reserve ${reserveAddress}: safety=${safetyScore.toFixed(2)}, ${details}`);
            }
...
//# sourceMappingURL=marketUtilizationTrigger.js.map