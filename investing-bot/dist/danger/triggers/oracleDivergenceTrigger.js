 ...
 */
export class OracleDivergenceTrigger {
    ...
    async check(_rpc, reserveAddress, reserve, context) {
        ...
        try {
            ...
            if (!reserve.tokenOraclePrice.valid) {
                logger.error(`[danger-detection] oracle price is flagged invalid/stale for mint ${mintAddress} — failing closed`);
                return {
                    safetyScore: 0.0,
            ...
            if (oraclePrice.lte(0)) {
                logger.error(`[danger-detection] oracle price is non-positive (${oraclePrice.toString()}) for mint ${mintAddress} while market price is ${marketPrice.toFixed(6)} — failing closed`);
                return {
                    safetyScore: 0.0,
                    triggerName: this.name,
                    reserveAddress,
            ...
            if (divergencePercent <= SAFE_DIVERGENCE_PERCENT) {
                ...
                const normalized = (divergencePercent - SAFE_DIVERGENCE_PERCENT) / (MAX_DIVERGENCE_PERCENT - SAFE_DIVERGENCE_PERCENT);
            ...
            if (safetyScore < 1.0) {
                logger.warn(`[danger-detection] oracle divergence for reserve ${reserveAddress}: safety=${safetyScore.toFixed(2)}, ${details}`);
            }
...
//# sourceMappingURL=oracleDivergenceTrigger.js.map