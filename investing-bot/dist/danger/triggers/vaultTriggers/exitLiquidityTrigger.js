 ...
 */
export class ExitLiquidityTrigger {
    name = 'exit-liquidity';
    async check(_rpc, reserveAddress, reserve, context) {
        ...
        try {
            ...
            const availableLiquidity = lamportsToDecimal(reserve.getFreelyAvailableLiquidityAmount(context.currentLedgerInstant, 0).toString(), decimals);
            ...
            const safetyScore = Math.max(0, Math.min(1.0, (liquidityRatio.toNumber() - MIN_LIQUIDITY_RATIO) / (1 - MIN_LIQUIDITY_RATIO)));
            ...
            if (safetyScore < 1.0) {
                logger.warn(`[danger-detection] exit liquidity for reserve ${reserveAddress}: safety=${safetyScore.toFixed(2)}, ${details}`);
            }
...
//# sourceMappingURL=exitLiquidityTrigger.js.map