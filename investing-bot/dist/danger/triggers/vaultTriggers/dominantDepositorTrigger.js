 ...
 */
export class DominantDepositorTrigger {
    name = 'dominant-depositor';
    async check(_rpc, reserveAddress, reserve, context) {
        ...
        try {
            ...
            const totalSupply = lamportsToDecimal(reserve.getEstimatedTotalSupply(currentLedgerInstant, 0).toString(), decimals);
            if (totalSupply.lte(0)) {
                ...
                logger.warn(`[danger-detection] dominant depositor for reserve ${reserveAddress}: safety=${safetyScore.toFixed(2)}, ${details}`);
            ...
            const configuredThreshold = context.maxVaultDominanceBps === undefined
                ? ''
 ...
 */
function computeDominanceSafetyScore(dominance, maxDominanceBps) {
