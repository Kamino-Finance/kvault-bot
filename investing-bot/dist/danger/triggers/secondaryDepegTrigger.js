 ...
 */
export const NON_USD_STABLECOIN_SYMBOLS = new Set([
    // non-USD currencies
    'EURC',
    'EURCV',
    'EUROE',
...
const DOLLAR_PEG = { kind: 'usd', priceUsd: 1.0 };
/** Every LST shares this target: worth at least the SOL backing it. */
const SOL_PAR_PEG = { kind: 'token', referenceMint: WRAPPED_SOL_MINT, minRatio: 1.0 };
/**
 ...
 */
export class SecondaryDepegTrigger {
    name = 'secondary-depeg';
    pegTargetOverrides;
    ...
    reportedUnpegged = new Set();
    reportedImplausible = new Set();
    constructor(pegTargetOverrides = new Map()) {
     ...
     * unpriced).
     */
    priceReferenceMints(reserveMints, tokenFlags) {
        const referenceMints = new Set();
     ...
     */
    resolvePegTarget(mint, tokenFlags) {
    ...
    async check(_rpc, reserveAddress, reserve, context) {
        ...
        try {
            ...
            const { divergenceBps, safeBps, maxBps, details } = peg.kind === 'usd'
            ...
            if (safetyScore < 1.0) {
                logger.warn(`[danger-detection] secondary market depeg for reserve ${reserveAddress}: safety=${safetyScore.toFixed(2)}, ${details}`);
            }
     ...
     */
    measureDollarPeg(mintAddress, pegPriceUsd, marketPrice, reserve) {
        ...
        const oracleBps = oraclePrice && oraclePrice.isFinite() && oraclePrice.gt(0)
        ...
        const details = `mint ${mintAddress}: market ${marketPrice.toFixed(6)} (${marketBps.toFixed(0)} bps) and ` +
     ...
     */
    measureTokenPeg(mintAddress, peg, marketPrice, context) {
        ...
        if (!referencePrice || !referencePrice.isFinite() || referencePrice.lte(0)) {
            throw new Error(`unusable secondary-market price for peg reference mint ${peg.referenceMint}: ${referencePrice ?? 'missing'}`);
        }
        ...
        const details = `mint ${mintAddress}: market ${observed.toFixed(6)} vs floor ${floor.toFixed(6)} × ${peg.referenceMint}, ` +
    ...
    reportUnpegged(mintAddress, flags) {
        ...
        if (flags?.isStablecoin) {
            logger.warn(`[danger-detection] ${this.name}: ${flags.symbol} (${mintAddress}) carries the stablecoin tag but is on the non-USD/yield-bearing exclusion list, so its peg value is NOT checked — only oracle-divergence covers it`);
            return;
        }
        logger.warn(`[danger-detection] ${this.name}: no peg is known for ${flags?.symbol ?? 'mint'} ${mintAddress} (not tagged stablecoin or lst), so its peg is NOT checked`);
    }
    /**
     ...
     */
    reportImplausiblePeg(mintAddress, observed, target, unit) {
        ...
        if (!this.reportedImplausible.has(key)) {
            ...
            logger.error(`[danger-detection] ${this.name}: mint ${mintAddress} trades at ${observed.toFixed(6)} against a ${target.toFixed(6)} ${unit} peg target (${ratio.toFixed(3)}x) — either a total depeg or a misclassified token; verify it before trusting this trigger's verdict`);
        }
        return true;
    }
}
//# sourceMappingURL=secondaryDepegTrigger.js.map