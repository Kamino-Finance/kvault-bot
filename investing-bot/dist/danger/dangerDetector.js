...
import { RISK_APPETITE_THRESHOLDS, } from './dangerTypes.js';
 ...
 */
export function getDefaultTriggers() {
    return [
 ...
 */
export function getDefaultVaultTriggers() {
 ...
 */
function combineSafetyScore(product, score) {
...
export class DangerDetector {
    ...
    vaultTriggers;
    blacklistPath;
    reinvestCooldownPasses;
    // reserve address string → remaining passes during which it must not receive new allocation.
    reinvestCooldownByReserve = new Map();
    constructor(triggers = getDefaultTriggers(), vaultTriggers = getDefaultVaultTriggers(), blacklistPath = DEFAULT_BLACKLIST_PATH, reinvestCooldownPasses = DEFAULT_REINVEST_COOLDOWN_PASSES) {
     ...
     * restarts the clock).
     */
    recordReinvestCooldown(reserve) {
     ...
     */
    getPriceReferenceMints(reserveMints, tokenFlags) {
     ...
     */
    async assessAllReserves(rpc, vaultsReservesMap, context) {
     ...
     */
    commitObservations(reserveAddresses, reservesMap) {
     ...
     */
    async filterDangerousReservesForVault(rpc, reservesMap, reserveAssessments, vaultInvestedInReserves, riskAppetite, context, maxVaultDominanceBps) {
        ...
        for (const [reserveAddress, vaultInvestedInReserve] of vaultInvestedInReserves) {
            ...
            if (combinedSafety < threshold) {
                ...
                logger.error(`[danger-detection] DANGER for reserve ${reserveAddress}: combined safety ${combinedSafety.toFixed(4)} < threshold ${threshold} (${riskAppetite}). Triggers: [${triggerDetails}]`);
            }
        }
        return dangerous;
    }
    ...
    clearPendingEvacuations(vaultAddress, reserveAddresses) {
        if (reserveAddresses.size === 0) {
            return;
        }
        updateBlacklistFile(this.blacklistPath, (blacklist) => {
            blacklist.pendingEvacuations = blacklist.pendingEvacuations.filter((entry) => entry.vault !== vaultAddress || !reserveAddresses.has(entry.reserve));
        });
    }
    /**
...
//# sourceMappingURL=dangerDetector.js.map