...
function validateBlacklistFile(parsed, path) {
    if (!isNonNullObject(parsed) || !Array.isArray(parsed.blacklistedReserves)) {
        throw new Error(`[danger-detection] Invalid blacklist file at ${path}: missing "blacklistedReserves" array. ` +
    ...
    blacklistedReserves.forEach((entry, index) => {
        ...
        if (!isNonEmptyString(entry.reserve) || !isAddress(entry.reserve)) {
            throw new Error(`${prefix}.reserve must be a non-empty Solana address string. ` +
                `Refusing to continue with corrupt safety state.`);
        }
        if (!isNonEmptyString(entry.triggerName)) {
            throw new Error(`${prefix}.triggerName must be a non-empty string. Refusing to continue with corrupt safety state.`);
        }
        ...
        if (!isNonEmptyString(entry.timestamp) || Number.isNaN(Date.parse(entry.timestamp))) {
            throw new Error(`${prefix}.timestamp must be a valid timestamp string. Refusing to continue with corrupt safety state.`);
        }
    });
    const pendingEvacuations = parsed.pendingEvacuations ?? [];
    if (!Array.isArray(pendingEvacuations)) {
        throw new Error(`[danger-detection] Invalid blacklist file at ${path}: "pendingEvacuations" must be an array. ` +
            `Refusing to continue with corrupt safety state.`);
    }
    pendingEvacuations.forEach((entry, index) => {
        ...
        if (!isNonEmptyString(entry.vault) || !isAddress(entry.vault)) {
            throw new Error(`${prefix}.vault must be a non-empty Solana address string. ` +
                `Refusing to continue with corrupt safety state.`);
        }
        if (!isNonEmptyString(entry.reserve) || !isAddress(entry.reserve)) {
            throw new Error(`${prefix}.reserve must be a non-empty Solana address string. ` +
    ...
    return {
        blacklistedReserves: blacklistedReserves,
        pendingEvacuations: pendingEvacuations,
    };
}
/**
 ...
 */
export function readBlacklistFile(path) {
    let raw;
    try {
        ...
        if (error.code === 'ENOENT') {
            ...
            if (!fs.existsSync(parentDir)) {
                throw new Error(`[danger-detection] Could not read blacklist file at ${path}: parent directory ${parentDir} does not exist. ` +
                    `Create the directory or mount the persistent volume before starting.`);
            }
            return { blacklistedReserves: [], pendingEvacuations: [] };
        }
        throw new Error(`[danger-detection] Could not read blacklist file at ${path}: ${error}. ` +
    ...
    try {
        ...
        throw new Error(`[danger-detection] Malformed blacklist file at ${path}: ${error}. Refusing to continue with corrupt safety state.`);
    }
 ...
 */
export function writeBlacklistFile(path, blacklist) {
 ...
 */
export function updateBlacklistFile(path, update) {
...
function acquireBlacklistLock(lockPath) {
    for (let attempt = 0; attempt < 2; attempt++) {
        try {
            ...
            throw new Error(`[danger-detection] Blacklist update already in progress (lock ${lockPath}, age ${Math.ceil(ageMs)}ms). Retry the operation.`);
        }
    }
 ...
 */
export function initBlacklistFile(path) {
    const lockPath = `${path}.lock`;
    const lockFd = acquireBlacklistLock(lockPath);
    try {
...
//# sourceMappingURL=blacklistStore.js.map