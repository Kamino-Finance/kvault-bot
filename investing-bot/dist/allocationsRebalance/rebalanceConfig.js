...
import { DEFAULT_DRIPPING_RATE_PERCENT, DEFAULT_ENFORCE_UTILIZATION_CAP, DEFAULT_MAX_VAULT_DOMINANCE_BPS, MAX_UTILIZATION_CHANGE_BPS, } from './consts.js';
import { FULL_BPS } from './utils/allocationHelper.js';
export const DEFAULT_REBALANCE_FREQUENCY_SECONDS = 3600;
export function readAllocationsConfig(configPath) {
    ...
    config.allocationsConfig.forEach((allocation, allocationIndex) => {
        ...
        validateOptionalBoolean(allocation.includeReservesSupplyFarmRewardsApy, `${allocationPath}.includeReservesSupplyFarmRewardsApy`);
        ...
        allocation.vaults.forEach((vault, vaultIndex) => {
            if (typeof vault !== 'string') {
                ...
                validateOptionalBoolean(vault.includeReservesSupplyFarmRewardsApy, `${vaultPath}.includeReservesSupplyFarmRewardsApy`);
                ...
                if (effectiveStrategy === RebalanceStrategy.FIXED_WEIGHTS) {
                    if (!vault.fixedReserves || !isReserveWeightFormat(vault.fixedReserves)) {
                        throw new Error(`[allocation-rebalance-loop] ${vaultPath}.fixedReserves must contain { reserve, weight } entries for FIXED_WEIGHTS`);
                    }
                }
                ...
                throw new Error(`[allocation-rebalance-loop] ${allocationPath}.vaults[${vaultIndex}] must be an object with fixedReserves for FIXED_WEIGHTS`);
            }
...
export function getAllocationConfig(strategy, config) {
 ...
 */
export function getVaultAddress(vaultEntry) {
    return typeof vaultEntry === 'string' ? vaultEntry : vaultEntry.vault;
}
/**
 * Get the rebalance frequency seconds for a vault entry
 */
export function getRebalanceFrequencySeconds(vaultAllocationConfig, vaultEntry) {
...
export function getFixedReservesWithConfig(vaultEntry) {
 ...
 */
export function getVaultStrategy(vaultEntry, defaultStrategy) {
 ...
 */
export function hasFixedReserves(vaultEntry) {
 ...
 */
export function isReserveWeightFormat(fixedReserves) {
    return (Array.isArray(fixedReserves) &&
        fixedReserves.length > 0 &&
        fixedReserves.every((entry) => typeof entry === 'object' && entry !== null && 'reserve' in entry && 'weight' in entry));
}
/**
 * Check if a vault entry uses FIXED_WEIGHTS strategy with reserve weights
 */
export function hasFixedWeights(vaultEntry) {
    return (typeof vaultEntry !== 'string' &&
 ...
 */
export function getReserveAddresses(fixedReserves) {
    if (!fixedReserves) {
        return [];
    }
...
export function validateConfiguredReserveMembership(vaultAddress, configuredReserves, vaultReserves) {
    ...
    for (const configuredReserve of configuredReserves) {
        if (!vaultReserveAddresses.has(configuredReserve)) {
            throw new Error(`[allocation-rebalance-loop] Configured reserve ${configuredReserve} is not part of vault ${vaultAddress}`);
        }
    }
}
/**
 ...
 */
export function getReserveWeights(vaultEntry) {
    if (typeof vaultEntry !== 'string' && hasFixedWeights(vaultEntry)) {
        return vaultEntry.fixedReserves;
    }
    return null;
}
/**
 ...
 */
export function getAllocationDryRun(vaultAllocationConfig, vaultEntry) {
...
export function getIncludeReservesSupplyFarmRewardsApy(vaultAllocationConfig, vaultEntry) {
 ...
 */
export function getRiskAppetiteMode(vaultAllocationConfig) {
 ...
 */
function validateRiskAppetiteMode(value) {
    if (value !== undefined && !Object.values(RiskAppetiteMode).includes(value)) {
        throw new Error(`[allocation-rebalance-loop] Invalid riskAppetiteMode "${value}" in allocation config. ` +
            `Must be one of: ${Object.values(RiskAppetiteMode).join(', ')}`);
    }
}
function validateRebalanceStrategy(value, fieldPath) {
    if (!Object.values(RebalanceStrategy).includes(value)) {
        throw new Error(`[allocation-rebalance-loop] Invalid ${fieldPath} "${value}". Must be one of: ${Object.values(RebalanceStrategy).join(', ')}`);
    }
}
function validatePositiveFiniteNumber(value, fieldPath) {
...
function validateReserveWeights(weights, fieldPath) {
    const seenReserves = new Set();
    weights.forEach((weight, index) => {
        if (typeof weight.reserve !== 'string' || !Number.isSafeInteger(weight.weight) || weight.weight < 0) {
            throw new Error(`[allocation-rebalance-loop] ${fieldPath}[${index}] must contain a reserve string and a non-negative safe-integer weight`);
        }
...
function validateFixedReserves(fixedReserves, fieldPath) {
    ...
    if (!fixedReserves.every((reserve) => typeof reserve === 'string')) {
        throw new Error(`[allocation-rebalance-loop] ${fieldPath} must contain only reserve address strings or only { reserve, weight } entries`);
    }
    const seenReserves = new Set();
 ...
 */
function validateUtilizationCapConfig(enforceUtilizationCap, maxUtilizationChangeBps) {
    if (enforceUtilizationCap !== undefined && typeof enforceUtilizationCap !== 'boolean') {
        throw new Error(`[allocation-rebalance-loop] Invalid enforceUtilizationCap "${enforceUtilizationCap}" in allocation config. ` +
            `Must be true or false.`);
    }
    if (maxUtilizationChangeBps !== undefined &&
        ...
        throw new Error(`[allocation-rebalance-loop] Invalid maxUtilizationChangeBps "${maxUtilizationChangeBps}" in allocation config. ` +
 ...
 */
function validateMaxVaultDominanceBps(maxVaultDominanceBps) {
    if (maxVaultDominanceBps !== undefined &&
        ...
        throw new Error(`[allocation-rebalance-loop] Invalid maxVaultDominanceBps "${maxVaultDominanceBps}" in allocation config. ` +
 ...
 */
export function getMaxVaultDominanceBps(vaultAllocationConfig, vaultEntry) {
 ...
 */
export function getDrippingRatePercent(vaultAllocationConfig, vaultEntry) {
 ...
 */
export function getEnforceUtilizationCap(vaultAllocationConfig, vaultEntry) {
 ...
 * value, falling back to the default
 */
export function getMaxUtilizationChangeBps(vaultAllocationConfig, vaultEntry) {
 ...
 */
export function printAllocationsConfig(config) {
    ...
    config.allocationsConfig.forEach((allocation, index) => {
        ...
        if (allocation.rebalanceFrequencySeconds) {
            console.log(`  ${chalk?.bold('Rebalance Frequency') || 'Rebalance Frequency'}: ${formatDuration(allocation.rebalanceFrequencySeconds)}`);
        }
        console.log(chalk?.yellow(`  🏦 Vaults (${allocation.vaults.length}):`) || `  🏦 Vaults (${allocation.vaults.length}):`);
        ...
        allocation.vaults.forEach((vault, vaultIndex) => {
            ...
            if (typeof vault === 'string') {
                ...
                console.log(`      ${chalk?.bold('Strategy') || 'Strategy'}: ${allocation.strategy} (inherited from allocation)`);
                if (allocation.rebalanceFrequencySeconds) {
                    console.log(`      ${chalk?.bold('Rebalance Frequency') || 'Rebalance Frequency'}: ${formatDuration(allocation.rebalanceFrequencySeconds)} (inherited)`);
                }
                if (allocation.includeReservesSupplyFarmRewardsApy !== undefined) {
                    console.log(`      ${chalk?.bold('Include Reserves Supply Farm Rewards APY') || 'Include Reserves Supply Farm Rewards APY'}: ${allocation.includeReservesSupplyFarmRewardsApy}`);
                }
                if (allocation.allocationDryRun !== undefined) {
                    console.log(`      ${chalk?.bold('Allocation Dry Run') || 'Allocation Dry Run'}: ${allocation.allocationDryRun}`);
                }
            }
            // Handle complex vault objects
                ...
                console.log(`      ${chalk?.bold('Type') || 'Type'}: Complex vault with ${fixedReserves.length} fixed reserve(s)`);
                console.log(`      ${chalk?.bold('Strategy') || 'Strategy'}: ${vault.strategy}`);
                if (vault.rebalanceFrequencySeconds) {
                    console.log(`      ${chalk?.bold('Rebalance Frequency') || 'Rebalance Frequency'}: ${formatDuration(vault.rebalanceFrequencySeconds)}`);
                }
                if (vault.includeReservesSupplyFarmRewardsApy !== undefined) {
                    console.log(`      ${chalk?.bold('Include Reserves Supply Farm Rewards APY') || 'Include Reserves Supply Farm Rewards APY'}: ${vault.includeReservesSupplyFarmRewardsApy}`);
                }
                if (vault.allocationDryRun !== undefined) {
                ...
                if (vault.reservesAllocationPercentageBPS !== undefined) {
                    ...
                    console.log(`      ${chalk?.bold('Fixed Reserves Allocation') || 'Fixed Reserves Allocation'}: ${percentage}% (${vault.reservesAllocationPercentageBPS} BPS)`);
                }
                if (vault.fixedReservesStrategy) {
                    console.log(`      ${chalk?.bold('Fixed Reserves Strategy') || 'Fixed Reserves Strategy'}: ${vault.fixedReservesStrategy}`);
                }
...
//# sourceMappingURL=rebalanceConfig.js.map