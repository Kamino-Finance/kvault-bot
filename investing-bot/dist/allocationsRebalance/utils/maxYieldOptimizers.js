...
import { computeOverallVaultApy, computeStabilizationFactorForVault, FULL_BPS, } from './allocationHelper.js';
 ...
 */
export function generateAllocationPoints(reservesCount, resolution, targetSum = 1) {
 ...
 */
export function generateRefinementRegionPoints(targetAllocation, radius, resolution) {
    ...
    function generateCombinations(index, currentAllocation, remainingSum) {
...
export function estimateResolutionsFromReservesCountAndCoarseResolution(reservesCount, coarseResolution) {
    const resolutions = {
 ...
 */
export function estimateRadiusMultiplierFromReservesCountAndResolution(reservesCount) {
 ...
 */
export function buildSearchCandidates(currentReservesAllocations, reserves, reservesWithMinAllocation) {
    const reservesWithCurrentAllocations = [];
    ...
    const reservesWithMinAllocationIndices = [];
 ...
 */
async function runCoarseToFineSearch({ candidatesCount, coarseResolution, mediumResolution, fineResolution, mediumRadius, fineRadius, initialScore, evaluate, score, isBetter, describe, verbose, }) {
    if (verbose) {
    ...
    logger.info(`[allocation-rebalance-loop] Coarse search completed. Best ${bestEvaluation ? describe(bestEvaluation) : 'result: none'}, evaluated ${coarsePoints.length} points`);
    if (!bestEvaluation || bestAllocation.length === 0 || !bestScore.isFinite()) {
        return null;
    }
    let selectedEvaluation = bestEvaluation;
    logger.info(`[allocation-rebalance-loop] Phase 2: Medium search with resolution ${mediumResolution}, radius ${mediumRadius}`);
    const mediumCandidates = secondBestAllocation.length > 0 ? [bestAllocation, secondBestAllocation] : [bestAllocation];
    const mediumPointKeys = new Set(mediumCandidates
    ...
    logger.info(`[allocation-rebalance-loop] Medium search completed. Best ${describe(selectedEvaluation)}, evaluated ${mediumPoints.length} points`);
    logger.info(`[allocation-rebalance-loop] Phase 3: Fine search with resolution ${fineResolution}, radius ${fineRadius}`);
    ...
    logger.info(`[allocation-rebalance-loop] Fine search completed. Best ${describe(selectedEvaluation)}, evaluated ${finePoints.length} points`);
    return { allocation: bestAllocation, evaluation: selectedEvaluation };
}
export async function gridSearchAllocationForMaxApyCoarseToFine(manager, vault, reserves, allocationWeightsSum, requestedCoarseResolution, currentLedgerInstant, vaultAUMTokens, investedInReservesTokens, shouldIncludeFarmRewards, farmsClient, mediumResolution, fineResolution, compoundingPeriods = 1, reservesWithMinAllocation = [], minTotalAllocationForSpecifiedReservesBPS = new Decimal(0), radiusMultiplier, // recommended: 2x coarse resolution for better coverage
farmsToFarmStateMap, pricesMap, verbose = false, 
...
allocationFilter, projectionContext, allInvestedInReservesTokens = investedInReservesTokens) {
    // Set default resolutions
    const resolutions = estimateResolutionsFromReservesCountAndCoarseResolution(investedInReservesTokens.size, requestedCoarseResolution);
    ...
    if (verbose) {
        logger.info(`[allocation-rebalance-loop] Coarse-to-fine search: coarse=${coarseResolution}, medium=${mediumRes}, fine=${fineRes}`);
        logger.info(`[allocation-rebalance-loop] Search radius: medium=${mediumRadius}, fine=${fineRadius} (multiplier=${radiusMultiplier})`);
    }
    // Build the candidate reserves from the supplied universe and the aligned min-allocation indices.
    const { reservesWithCurrentAllocations, reservesWithMinAllocationIndices } = buildSearchCandidates(manager.getVaultAllocations(vault), reserves, reservesWithMinAllocation);
    ...
    async function evaluateAllocation(allocation) {
        ...
        try {
            return await withTimeout(computeOverallVaultApy(allocation, reservesWithCurrentAllocations, allocationWeightsSum, vaultAUMTokens, allInvestedInReservesTokens, currentLedgerInstant, compoundingPeriods, shouldIncludeFarmRewards, localFarmsClient, farmsToFarmStateMap, pricesMap, verbose, projectionContext), 120000, // 120 second timeout
    ...
    if (!searchResult) {
        logger.warn(`[allocation-rebalance-loop] Grid search found no feasible allocation (every evaluation failed). Falling back to current weights.`);
    ...
    const bestAllocationWithReserves = new Map();
    ...
    logger.info(`[allocation-rebalance-loop] Coarse-to-fine best allocation: ${bestAllocationWithReserves.toString()} with yield: ${searchResult.evaluation.toString()}`);
    return {
 ...
 */
export async function gridSearchAllocationForMinStabilizationFactorCoarseToFine(manager, vault, reserves, allocationWeightsSum, requestedCoarseResolution, currentLedgerInstant, vaultAUMTokens, investedInReservesTokens, shouldIncludeFarmRewards, farmsClient, mediumResolution, fineResolution, compoundingPeriods = 1, reservesWithMinAllocation = [], minTotalAllocationForSpecifiedReservesBPS = new Decimal(0), radiusMultiplier, // recommended: 2x coarse resolution for better coverage
farmsToFarmStateMap, pricesMap, verbose = false, projectionContext, allInvestedInReservesTokens = investedInReservesTokens) {
    // Set default resolutions
    const resolutions = estimateResolutionsFromReservesCountAndCoarseResolution(investedInReservesTokens.size, requestedCoarseResolution);
    const coarseResolution = resolutions.coarseResolution;
    const mediumRes = mediumResolution || resolutions.mediumResolution;
    ...
    async function evaluateAllocation(allocation) {
        ...
        // Wrap the APY computation with timeout to prevent hanging
        try {
            return await withTimeout(computeStabilizationFactorForVault(allocation, reservesWithCurrentAllocations, allocationWeightsSum, vaultAUMTokens, allInvestedInReservesTokens, currentLedgerInstant, compoundingPeriods, shouldIncludeFarmRewards, localFarmsClient, farmsToFarmStateMap, pricesMap, verbose, projectionContext), 120000, // 120 second timeout
    ...
    const searchResult = await runCoarseToFineSearch({
        ...
        describe: (result) => `stabilization factor: ${result.stabilizationFactor.toString()} and APY: ${result.apy.toString()}`,
        verbose,
    });
    if (!searchResult) {
        logger.warn(`[allocation-rebalance-loop] Stabilization search found no feasible allocation (every evaluation failed). Falling back to current weights.`);
        const current = new Map();
    ...
    logger.info(`[allocation-rebalance-loop] Coarse-to-fine best allocation: ${bestAllocationWithReserves.toString()} with stabilization factor: ${searchResult.evaluation.stabilizationFactor.toString()}`);
    return {
        reservesWithAllocation: bestAllocationWithReserves,
...
//# sourceMappingURL=maxYieldOptimizers.js.map