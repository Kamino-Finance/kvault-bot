...
import { computeVaultTargetAllocation, evaluateReserveSupplyYieldWithNewAllocation, getSimulatedReserveSupplyFarmAPY, } from './allocationHelper.js';
/**
 ...
 */
export async function buildReservesAllocationLog(kaminoManager, kaminoVault, vaultsReserves, allocation, currentLedgerInstant, shouldIncludeFarmRewards, farmsClient, farmsToFarmStateMap, pricesMap, allVaultReserves = vaultsReserves, forcedZeroReserves = new Set()) {
    ...
    for (const reserveAlloc of vaultState.vaultAllocationStrategy) {
        ...
        const toInvestInReserveTokens = targetAllocation.targetReservesAllocation.get(reserveAlloc.reserve) ?? new Decimal(0);
        const reserveCollExchangeRate = kReserve.getEstimatedCollateralExchangeRate(currentLedgerInstant, 0);
        const reserveAllocationLiquidityAmountLamports = new Decimal(reserveAlloc.ctokenAllocation.toString()).div(reserveCollExchangeRate);
        const reserveAllocationLiquidityAmountTokens = lamportsToDecimal(reserveAllocationLiquidityAmountLamports, vaultState.tokenMintDecimals.toNumber());
        const totalSupplyTokens = lamportsToDecimal(kReserve.getEstimatedTotalSupply(currentLedgerInstant, 0), vaultState.tokenMintDecimals.toNumber());
        const simulatedReserveSupplyApy = evaluateReserveSupplyYieldWithNewAllocation(kReserve, toInvestInReserveTokens, investedInReservesTokens.get(kReserve.address), currentLedgerInstant, 1);
        ...
        if (shouldIncludeFarmRewards && kReserve.state.farmCollateral !== DEFAULT_PUBLIC_KEY) {
            simulatedReserveSupplyFarmAPY = await getSimulatedReserveSupplyFarmAPY(toInvestInReserveTokens, investedInReservesTokens.get(kReserve.address), kReserve, farmsClient, farmsToFarmStateMap, pricesMap);
        }
