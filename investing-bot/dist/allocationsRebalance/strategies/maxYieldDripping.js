...
import { estimateResolutionsFromReservesCountAndCoarseResolution, gridSearchAllocationForMaxApyCoarseToFine, } from '../utils/maxYieldOptimizers.js';
import { computeOverallVaultApy, computeVaultTargetAllocation, FULL_BPS, } from '../utils/allocationHelper.js';
import { calculateTotalWeight, getTargetTokenConstraintType, normalizeWeightsToTotal, } from '../utils/allocationConstraints.js';
import { DEFAULT_DRIPPING_RATE_PERCENT, DEFAULT_ENFORCE_UTILIZATION_CAP, MAX_UTILIZATION_CHANGE_BPS, } from '../consts.js';
 ...
 */
export function computeDrippedWeights(currentWeights, targetWeights, dripRatePercent) {
    const dripRate = new Decimal(dripRatePercent).div(100);
    const drippedWeights = new Map();
...
export async function getMaxYieldDrippingAllocationRebalanceIxs({ kaminoManager, kaminoVault, vaultsReserves, signer, currentLedgerInstant, gridSearchResolution, shouldIncludeFarmRewards, drippingRatePercent = DEFAULT_DRIPPING_RATE_PERCENT, compoundingPeriods = 1, farmsToFarmStateMap, pricesMap, verbose = false, enforceUtilizationCap = DEFAULT_ENFORCE_UTILIZATION_CAP, maxUtilizationChangeBps = MAX_UTILIZATION_CHANGE_BPS, allVaultReserves = vaultsReserves, preservedReserves = new Set(), forcedZeroReserves = new Set(), }) {
    ...
    if (healthyTotalWeight.lte(0)) {
        logger.info(`[allocation-rebalance-loop] Dripping vault ${kaminoVault.address.toString()}: healthy allocation weight total is 0 (unallocated vault or all weight on blacklisted reserves) - nothing to drip`);
        return {
            ixns: [],
            bestAllocation: {
    ...
    if (enforceUtilizationCap) {
        ...
        const baseConstraintsByReserve = new Map();
        for (const [reserveAddress, weight] of currentWeights) {
            ...
            baseConstraintsByReserve.set(reserveAddress, buildReserveConstraintsBase(reserveAddress, reserve, weight, investedTokens, currentLedgerInstant));
        }
        utilizationFilter = (allocation, reservesWithAllocations) => {
            ...
            for (let i = 0; i < reservesWithAllocations.length; i++) {
                proposedWeights.set(reservesWithAllocations[i].reserve.address, allocation[i].div(allocWeightsSum).mul(healthyTotalWeight));
            }
            ...
            for (let i = 0; i < reservesWithAllocations.length; i++) {
                ...
                const simulatedUtil = new Decimal(reserve.calcSimulatedUtilizationRatio(tokenDeltaLamports, action, currentLedgerInstant, 0));
    ...
    const maxYieldAllocation = await gridSearchAllocationForMaxApyCoarseToFine(kaminoManager, vaultContext.vaultState, vaultsReserves, healthyTotalWeight, gridSearchResolution, currentLedgerInstant, vaultContext.vaultAUMTokens, vaultContext.investedInReservesTokensMap, shouldIncludeFarmRewards, farmsClient, undefined, // mediumResolution - use default
    undefined, // fineResolution - use default
    compoundingPeriods, [], // reservesWithMinAllocation - fixed reserves are not supported for dripping
    new Decimal(0), // minTotalAllocationForSpecifiedReservesBPS
    undefined, // radius multiplier - use default
    farmsToFarmStateMap, pricesMap, verbose, utilizationFilter, vaultContext.allocationProjectionContext, vaultContext.allInvestedInReservesTokensMap);
    ...
    const drippedWeights = computeDrippedWeights(currentWeights, maxYieldAllocation.reservesWithAllocation, drippingRatePercent);
    ...
    const { fineResolution } = estimateResolutionsFromReservesCountAndCoarseResolution(vaultContext.investedInReservesTokensMap.size, gridSearchResolution);
    ...
    if (shouldStayPut) {
        logger.info(`[allocation-rebalance-loop] Dripping vault ${kaminoVault.address.toString()}: max target gap ${maxTargetGap} <= stay-put gate ${stayPutGate} (fineResolution=${fineResolution}) - staying put`);
    }
    ...
    if (chosenTotalWeight.lte(0)) {
        logger.error(`[allocation-rebalance-loop] Dripping vault ${kaminoVault.address.toString()}: chosen allocation weight total is ${chosenTotalWeight} despite healthy total ${healthyTotalWeight}. Staying put.`);
        return {
            ixns: [],
            bestAllocation: {
                reservesWithAllocation: currentWeights,
    ...
    const normalizedAllocation = reservesWithCurrentAllocations.map(({ reserve }) => (reservesWithAllocation.get(reserve.address) ?? new Decimal(0)).div(chosenTotalWeight));
    let chosenApy;
    try {
        chosenApy = await computeOverallVaultApy(normalizedAllocation, reservesWithCurrentAllocations, chosenTotalWeight, vaultContext.vaultAUMTokens, vaultContext.allInvestedInReservesTokensMap, currentLedgerInstant, compoundingPeriods, shouldIncludeFarmRewards, farmsClient, farmsToFarmStateMap, pricesMap, verbose, vaultContext.allocationProjectionContext);
    }
    catch (error) {
        logger.error(`[allocation-rebalance-loop] Failed to compute the final APY for dripping vault ${kaminoVault.address.toString()}: ${error}. Staying put.`);
        return {
            ixns: [],
            bestAllocation: {
                reservesWithAllocation: currentWeights,
    ...
    const allocationRebalanceIxs = await buildAllocationRebalanceInstructions(kaminoManager, kaminoVault, vaultContext.vaultState, vaultsReserves, bestAllocation, signer, currentLedgerInstant, allVaultReserves, preservedReserves, forcedZeroReserves);
    return {
        ixns: allocationRebalanceIxs,
        bestAllocation,
    };
}
//# sourceMappingURL=maxYieldDripping.js.map