...
import { lamportsToDecimal, ReserveAllocationConfig, U64_MAX, } from '@kamino-finance/klend-sdk';
import { logger } from 'kvaults-investing-bot-logger';
import BN from 'bn.js';
import { computeVaultTargetAllocation, getVaultTotalAllocationsWeights, } from '../utils/allocationHelper.js';
import { applyReserveConstraints, calculateReserveAllocationWeightBounds, getTargetTokenConstraintType, normalizeWeightsToTotal, redistributeWeightDifferenceWithinBounds, } from '../utils/allocationConstraints.js';
import { getAllocationCapInTokensOrDefault, getVaultAllocationForReserve, shouldUpdateAllocation, } from '../vaultUtils.js';
import { getInvestedReservesForUniverse } from '../rebalanceUniverse.js';
function getRebalancedAllocationWeight(vaultState, preservedReserves) {
...
function getRebalancedVaultAUMTokens(totalVaultAUMTokens, investedInReservesTokens, preservedReserves) {
 ...
 */
export async function getVaultContext(kaminoManager, kaminoVault, optimizationReserves, currentLedgerInstant, allVaultReserves = optimizationReserves, preservedReserves = new Set(), forcedZeroReserves = new Set()) {
    ...
    const vaultAUMTokens = getRebalancedVaultAUMTokens(totalVaultAUMTokens, vaultHoldings.investedInReserves, preservedReserves);
    const investedInReservesTokensMap = getInvestedReservesForUniverse(vaultHoldings.investedInReserves, optimizationReserves);
 ...
 */
export function logAllocationResult(kaminoVault, strategyName, allocation, reservesWithMinAllocation, stabilizationFactor) {
 ...
 */
export function buildReserveConstraintsBase(reserveAddress, reserve, currentWeight, investedTokensInReserve, currentLedgerInstant) {
    ...
    return {
        ...
        currentTotalSupplyInReserveTokens: lamportsToDecimal(reserve.getEstimatedTotalSupply(currentLedgerInstant, 0).toString(), decimals),
        availableLiquidityInReserveTokens: lamportsToDecimal(reserve.getFreelyAvailableLiquidityAmount(currentLedgerInstant, 0).toString(), decimals),
        ...
        reserveWithdrawalCapCurrentTokens: withdrawalCapDisabled
            ? null
            : lamportsToDecimal(reserve.getDepositWithdrawalCapCurrent(Number(currentLedgerInstant.blockTime)).toString(), decimals),
    };
}
/**
 * Builds allocation rebalance instructions from allocation results
 */
export async function buildAllocationRebalanceInstructions(kaminoManager, kaminoVault, vaultState, vaultsReserves, proposedAllocation, signer, currentLedgerInstant, allVaultReserves = vaultsReserves, preservedReserves = new Set(), forcedZeroReserves = new Set()) {
    ...
    const rebalancedVaultAUMTokens = getRebalancedVaultAUMTokens(totalVaultAUMTokens, investedInReservesTokens, preservedReserves);
    ...
    const adjustedAllocation = new Map();
    ...
    for (const [reserveToUpdate, targetWeight] of adjustedAllocation) {
        ...
        const constraints = {
            ...buildReserveConstraintsBase(reserveToUpdate, reserve, currentWeight, investedTokensInReserve, currentLedgerInstant),
            targetWeight,
        };
        weightBounds.set(reserveToUpdate, calculateReserveAllocationWeightBounds(constraints, aumTokens, totalTargetWeight));
        // Apply constraints
        ...
        if (constraintResult.constraintHit) {
            ...
            logger.info(`Reserve ${reserveToUpdate} ${constraintResult.constraintType} constraint: target=${targetWeight}, adjusted=${constraintResult.adjustedWeight}, delta=${constraintResult.adjustedWeight.sub(targetWeight)}`);
        }
    }
    ...
    if (totalTargetWeight.gt(0)) {
        ...
        const redistributionResult = redistributeWeightDifferenceWithinBounds(adjustedAllocation, totalTargetWeight, weightBounds);
        if (!redistributionResult.isFeasible) {
            logger.warn(`[allocation-rebalance-loop] Cannot conserve total allocation weight ${totalTargetWeight} within reserve deposit and withdrawal bounds. Staying put.`);
            return [];
        }
    ...
    for (const [reserveAddress] of finalAllocation) {
        ...
        if (!reserve || !allocForReserve) {
            logger.warn(`[allocation-rebalance-loop] Cannot validate canonical target for reserve ${reserveAddress}. Staying put.`);
            return [];
        }
        const constraints = buildReserveConstraintsBase(reserveAddress, reserve, new Decimal(allocForReserve.targetAllocationWeight.toString()), investedInReservesTokens.get(reserveAddress) ?? new Decimal(0), currentLedgerInstant);
        ...
        if (constraintType !== 'none') {
            logger.warn(`[allocation-rebalance-loop] Canonical target ${targetTokens} for reserve ${reserveAddress} exceeds its ${constraintType} capacity. Staying put.`);
            return [];
        }
    }
    ...
    for (const [reserveToUpdate, adjustedWeight] of finalAllocation) {
        ...
        if (!weightDelta.eq(0)) {
            ...
            const reserveAllocationConfig = new ReserveAllocationConfig(reserveWithConfig, Math.round(adjustedWeight.toNumber()), // Ensure integer weight
            allocationCapTokens);
            const shouldUpdateAlloc = shouldUpdateAllocation(vaultState, reserveAllocationConfig);
            if (shouldUpdateAlloc) {
                const updateAllocationIxs = await kaminoManager.updateVaultReserveAllocationIxs(kaminoVault, reserveAllocationConfig, signer);
