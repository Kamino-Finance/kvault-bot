...
import { KswapSdk, NON_COMPOSABLE_ROUTERS, SUPPORTED_ROUTER_TYPES, } from '@kamino-finance/kswap-sdk';
...
import { base64EncodeTx, MAX_LOCKED_ACCOUNTS, maxLockedAccounts, sendAndConfirmTransactionV0, simulateTxIsSuccessful, uniqueAccounts, WRAPPED_SOL_MINT, } from 'kvaults-investing-bot-tx/instruction';
...
import { SwapMode } from '../libs/actions/swap.js';
export class KSwapWrapper {
    payer;
    kswapSdk;
    routers;
    composableRouters;
    constructor(payer, c, kswapApiBaseUrl, kswapApiKey, excludeRouters) {
        this.payer = payer;
        this.kswapSdk = new KswapSdk(kswapApiBaseUrl, c.getRpc(), c.getWsRpc(), kswapApiKey, {
            logger: logger,
    ...
    async swap(c, inputToken, outputToken, inputAmountLamports, swapConfig, userLuts, description = 'KSwap') {
        let { destinationTokenAccount } = swapConfig;
        const solAccountSetupIxs = [];
        const additionalSigners = [];
        const solAccountCleanupIxs = [];
        ...
        if (outputToken === WRAPPED_SOL_MINT && swapConfig?.wrapAndUnwrapSol && !destinationTokenAccount) {
            ...
            const tempSolSetupAccIxs = await createTokenAccountInstructions(c.getRpc(), tempSolAcc, outputToken, this.payer, TOKEN_PROGRAM_ADDRESS);
        ...
        const responses = await this.swapTxFindSuitableRoutes(inputToken, outputToken, inputAmountLamports, finalSwapConfig, description);
        ...
        for (let i = 0; i < responses.length; i++) {
            ...
            if (swapResponse.swapTxs.swapIxs.length === 0) {
                logger.error(`${descriptionWithIndex} KSwap returned an empty swap instruction list for route, likely parameters passed are invalid`);
                continue;
            }
            if (swapResponse.swapResponse.swapOutAmountLamports.eq(0)) {
                logger.error(`${descriptionWithIndex} KSwap returned a swap response with 0 swap out amount, trying next route`);
                continue;
            }
            ...
            const { preAssertBalancesIxs, postAssertBalancesIxs } = swapConfig.balanceGuard
                ? await this.getAssertSwapBalancesIxs(swapResponse.swapResponse.swapMaxInAmountLamports, swapResponse.swapResponse.swapMinOutAmountLamports, { mint: inputToken, tokenProgramOwner: swapConfig.balanceGuard.inputTokenProgramOwner }, { mint: outputToken, tokenProgramOwner: swapConfig.balanceGuard.outputTokenProgramOwner }, destinationTokenAccount)
            ...
            const simulationIsSuccessful = await simulateTxIsSuccessful(c.getRpc(), ixs, this.payer.address, luts, descriptionWithIndex);
            if (!simulationIsSuccessful) {
                logger.error(`${descriptionWithIndex} KSwap returned a simulation that failed, trying next route`);
                continue;
            }
            try {
                const tx = await sendAndConfirmTransactionV0(c, this.payer, ixs, luts, additionalSigners, descriptionWithIndex, { blockhash, reportSample: false, sendIfSimulationFailed: true });
                return {
                    tx,
    ...
    async swapTxFindSuitableRoutes(inputMint, outputMint, amountLamports, swapConfig, description) {
        ...
        const preferredMaxAccounts = swapConfig.txAccounts
            ...
            : undefined;
        logger.info(`${description} Requesting KSwap /all-routes with swap type ${swapMode} from ${inputMint} to ${outputMint} with ${amountLamports} lamports, slippage (bps): ${swapConfig.slippageBps}, preferred max accounts: ${preferredMaxAccounts}`);
        ...
        logger.info(`${descriptionWithTraceId} returned ${response.routes.length} (${response.routes.map((r) => r.routerType)}) routes`);
        if (response.routes.length === 0) {
            logger.warn(`${descriptionWithTraceId} returned no routes for swap type ${swapMode} from ${inputMint} to ${outputMint} with ${amountLamports} lamports`);
            throw new NoSwapRoutesFoundError();
        }
        if (preferredMaxAccounts === undefined) {
            response.routes.sort(getRouteOutputComparator(swapMode));
            logger.info(`${descriptionWithTraceId} no preferred max accounts specified, returning all ${response.routes.length} routes in sorted order (${response.routes.map((r) => r.routerType)})`);
        ...
        const within = response.routes.filter((route) => {
            ...
            const allAccounts = uniqueAccounts(swap.swapTxs.swapIxs, swap.swapLookupTableAccounts, ...(swapConfig.txAccounts || []));
        ...
        if (notWithin.length > 0) {
            logger.info(`${descriptionWithTraceId} filtered ${notWithin.length} routes which exceeded the SVM limit of ${MAX_LOCKED_ACCOUNTS} locked accounts (${notWithin.map((x) => `${x.routerType}:${x.accounts}`)})`);
        }
        if (within.length === 0) {
            logger.warn(`${descriptionWithTraceId} no routes were found within the SVM limit of ${MAX_LOCKED_ACCOUNTS} locked accounts`);
            throw new NoSwapRoutesFoundError();
        }
    ...
    async getAssertSwapBalancesIxs(swapAmountInLamports, minSwapAmountOutLamports, inputToken, outputToken, destinationTokenAccount) {
        if (!inputToken.tokenProgramOwner && !inputToken.ata) {
            throw new Error('KSwapWrapper::getAssertSwapBalancesIxs: at least one of inputToken.tokenProgramOwner or inputToken.ata must be provided');
        }
        if (!outputToken.tokenProgramOwner && !outputToken.ata) {
            throw new Error('KSwapWrapper::getAssertSwapBalancesIxs: at least one of outputToken.tokenProgramOwner or outputToken.ata must be provided');
        }
        ...
        const { preIx, postIx } = await this.kswapSdk.getLedgerIxs(this.payer, inputTa, outputTa, new BN(swapAmountInLamports.toString()), // maxInputAmountChange
    ...
    getRouters(composableRoutersOnly) {
...
export function swapTxFromRoute(route, swapConfig) {
    const swapMode = swapConfig.swapMode ?? SwapMode.ExactIn;
...
export function assertSwapPriceWithinTolerance(response, swapMode, guard) {
    ...
    if (!Number.isInteger(inputTokenDecimals) ||
    ...
    if ((swapMode ?? SwapMode.ExactIn) === SwapMode.ExactIn) {
        ...
        if (response.swapMinOutAmountLamports.lt(minOutputBaseUnits)) {
            throw new Error(`KSwap route fails price guard: guaranteed output ${response.swapMinOutAmountLamports.toString()} is below ${minOutputBaseUnits.toString()} base units`);
        }
        return;
    }
    ...
    if (response.swapMaxInAmountLamports.gt(maxInputBaseUnits)) {
        throw new Error(`KSwap route fails price guard: maximum input ${response.swapMaxInAmountLamports.toString()} exceeds ${maxInputBaseUnits.toString()} base units`);
    }
}
function getRouteOutputComparator(swapMode) {
...
//# sourceMappingURL=kSwapWrapper.js.map