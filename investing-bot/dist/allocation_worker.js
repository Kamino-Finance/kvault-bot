...
async function startAllocationWorker() {
    let cleanup = () => { };
    try {
        const cluster = workerData.cluster;
        logger.info('[allocation-worker] Starting allocation loop in worker thread');
        // Initial memory report
        const initialMemory = getMemoryUsage();
        logger.info(`[allocation-worker] Initial memory usage: ${initialMemory.heapUsed}MB heap, ${initialMemory.rss}MB RSS`);
...
//# sourceMappingURL=allocation_worker.js.map