...
var LoopName;
(function (LoopName) {
    LoopName["Invest"] = "invest-loop";
    LoopName["AllocationRebalance"] = "allocation-rebalance-loop";
})(LoopName || (LoopName = {}));
const LOOP_HEALTH_CHECK_INTERVAL_MS = 15_000;
let loopHealthMonitor;
let lastReadinessState;
const loopHealth = new Map();
function configureLoopHealth(expectedLoops) {
...
function markLoopHeartbeat(loopName) {
...
function markLoopUnhealthy(loopName) {
    const state = loopHealth.get(loopName);
    if (!state) {
        return;
    }
...
async function startLoopAsync(loopName, loopFunction) {
...
function startAllocationWorker(cluster, previousRestartCount = 0) {
...
async function main() {
    const cluster = getEnvOrDefault('CLUSTER', 'mainnet-beta');
    ...
    if (investLoop) {
        ...
        startLoopAsync(LoopName.Invest, async () => {
            ...
            await recursiveTryCatch(() => runInvestLoop(cluster, () => markLoopHeartbeat(LoopName.Invest)), `[${LoopName.Invest}]`);
        }).catch((e) => {
