 ...
 */
function requireBlacklistPath(path) {
    if (path.trim().length === 0) {
        throw new Error('DANGER_BLACKLIST_PATH is set but empty — refusing to operate on an unusable blacklist path. Set it to the intended file or unset it to use the default.');
    }
    return path;
}
async function main() {
    ...
    danger
        ...
        console.log(created ? `Created empty blacklist at ${path}.` : `Blacklist already exists at ${path}; leaving it untouched.`);
    });
    danger
        .command('list')
    ...
    danger
        ...
        .option('--path <path>', 'Path to blacklist file', defaultBlacklistPath())
        .action((reserveAddress, opts) => {
        const path = requireBlacklistPath(opts.path);
        const removed = updateBlacklistFile(path, (blacklist) => {
            const before = blacklist.blacklistedReserves.length;
            blacklist.blacklistedReserves = blacklist.blacklistedReserves.filter((entry) => entry.reserve !== reserveAddress);
