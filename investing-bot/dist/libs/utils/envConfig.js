...
import { DEFAULT_BLACKLIST_PATH, DEFAULT_LOOP_INTERVAL_MS, DEFAULT_MIN_INVEST_TOKENS, DEFAULT_MIN_SECONDS_SINCE_LAST_INVEST, } from './consts.js';
import { getEnvOrDefault, getEnv, getEnvOrThrowInProduction, getEnvOrDefaultKey, getEnvOrDefaultNum, getEnvOrDefaultBool, getEnvOrDefaultJson, getAllEnvsByPrefix, } from './env.js';
import { parseRpcEndpointsConfigEnvs } from './rpcConfig.js';
export { parseRpcEndpointsConfigEnvs } from './rpcConfig.js';
export function fromBaseEnv() {
    ...
    const MIN_SECONDS_SINCE_LAST_INVEST = getEnvOrDefaultNum('MIN_SECONDS_SINCE_LAST_INVEST', DEFAULT_MIN_SECONDS_SINCE_LAST_INVEST);
    ...
    if (PRIORITY_MICRO_LAMPORTS_PER_CU_DEFAULT < PRIORITY_MICRO_LAMPORTS_PER_CU_MIN ||
        PRIORITY_MICRO_LAMPORTS_PER_CU_DEFAULT > PRIORITY_MICRO_LAMPORTS_PER_CU_MAX) {
        throw new Error(`Inconsistent priority fee bounds: ${PRIORITY_MICRO_LAMPORTS_PER_CU_MIN} <= ${PRIORITY_MICRO_LAMPORTS_PER_CU_DEFAULT} <= ${PRIORITY_MICRO_LAMPORTS_PER_CU_MAX}`);
    }
     ...
     */
    const RPC_MULTICAST_ENDPOINTS = getRpcMulticastEndpoints();
    ...
    const MARKET_PRICE_MAX_AGE_SECONDS = getEnvOrDefaultNum('MARKET_PRICE_MAX_AGE_SECONDS', DEFAULT_MARKET_PRICE_MAX_AGE_SECONDS);
    ...
    if (DANGER_BLACKLIST_PATH.trim().length === 0) {
        throw new Error('DANGER_BLACKLIST_PATH is set but empty — refusing to start with an unusable blacklist path (danger enforcement would silently read an empty blacklist)');
    }
    return {
        rpcEndpointsConfig,
        rpcEndpoint,
...
export function parseEnvList(vaults) {
...
function getOrDerivePriorityFeePercentile(percentileString) {
...
function validateNonNegativeSafeInteger(value, name) {
    if (!Number.isSafeInteger(value) || value < 0) {
        throw new Error(`${name} must be a non-negative safe integer`);
    }
}
function validateNonNegativeFiniteNumber(value, name) {
    if (!Number.isFinite(value) || value < 0) {
        throw new Error(`${name} must be a non-negative finite number`);
    }
}
function validateSlippageBps(value, name) {
 ...
 * RPC_MULTICAST_ENDPOINT_IRONFORGE='https://...'
 */
export function getRpcMulticastEndpoints() {
    return [
...
function upperSnakeCaseToKebabCase(input) {
