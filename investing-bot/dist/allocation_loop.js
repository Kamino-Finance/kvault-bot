...
import { fetchAllMaybeFarmState } from '@kamino-finance/farms-sdk';
import { getLut } from './libs/lut.js';
import { readSecret } from './libs/utils/secret.js';
import { getAllocationDryRun, getDrippingRatePercent, getEnforceUtilizationCap, getMaxUtilizationChangeBps, getFixedReservesWithConfig, getIncludeReservesSupplyFarmRewardsApy, getRebalanceFrequencySeconds, getReserveAddresses, getReserveWeights, getVaultAddress, getVaultStrategy, printAllocationsConfig, readAllocationsConfig, validateConfiguredReserveMembership, } from './allocationsRebalance/rebalanceConfig.js';
...
import { DangerCoordinator } from './danger/dangerCoordinator.js';
...
async function heartbeatSleepSeconds(seconds, heartbeat) {
    return interruptibleSleep(seconds * 1000, heartbeat);
}
export async function runAllocationLoop(cluster, heartbeat) {
    ...
    const allocationsConfig = readAllocationsConfig(envConfig.allocationConfigPath);
    printAllocationsConfig(allocationsConfig);
    const allocationAdmin = await createKeyPairSignerFromBytes(Buffer.from(JSON.parse(readSecret('allocation_admin', 'ALLOCATION_ADMIN_SECRET_PATH'))));
    logger.info(`[allocation-rebalance-loop] allocationAdmin ${allocationAdmin.address.toString()}`);
    await runAllocationRebalanceLoop(allocationsConfig, c, kaminoManager, allocationAdmin, envConfig.gridSearchResolution, envConfig.verbose, envConfig.allocationDryRun, envConfig.blacklistPath, envConfig.marketPriceMaxAgeSeconds, heartbeat, refreshKaminoManager);
}
export async function runAllocationRebalanceLoop(allocationsConfig, c, initialKaminoManager, allocationAdmin, gridSearchResolution, verbose, dryRun, blacklistPath, marketPriceMaxAgeSeconds, heartbeat, refreshKaminoManager) {
    ...
    const allTokensMintsIncludingFarms = new Set();
    const farmToFarmStateMap = new Map();
    if (!allocationsConfig || !allocationsConfig.allocationsConfig || allocationsConfig.allocationsConfig.length === 0) {
        ...
        const lastRebalanceTimestampInSecondsPerVault = new Map();
        ...
        allocationsConfig.allocationsConfig.forEach((allocation) => {
            logger.info(`[allocation-rebalance-loop] ${allocation.strategy}: ${allocation.vaults
        ...
        while (!isProcessShuttingDown()) {
            ...
            const kaminoVaults = await withTimeout(kaminoManager.getVaults(allVaultsAddresses), RPC_REQUEST_TIMEOUT_MS, '[allocation-rebalance-loop] fetch vaults');
            ...
            const vaultsReserves = await loadVaultsReservesInBatches(kaminoManager, vaultStates, '[allocation-rebalance-loop] load vault reserves', hasOperationalCheckpoint ? heartbeat : undefined);
            const vaultsReservesMap = new Map();
            ...
            let dangerResult;
            try {
                dangerResult = await withTimeout(dangerCoordinator.detectAndRespond(allocationsConfig, kaminoVaultsMap, vaultsReservesMap, vaultsReserves, dryRun, heartbeat), Math.max(allVaultsAddresses.length, 1) * RPC_REQUEST_TIMEOUT_MS * 4, '[allocation-rebalance-loop] danger detection pass');
            }
            catch (e) {
            ...
            let reservesSupplyFarms = new Set();
            ...
            if (reservesSupplyFarms.size > 0) {
                ...
                const farmsStates = await withTimeout(fetchAllMaybeFarmState(c.getRpc(), farmsList), RPC_REQUEST_TIMEOUT_MS, '[allocation-rebalance-loop] fetch farm states');
            ...
            for (const allocation of allocationsConfig.allocationsConfig) {
                ...
                for (const vaultEntry of vaultEntries) {
                    ...
                    if (!dangerDirective || dangerDirective.skip) {
                        logger.warn(`[allocation-rebalance-loop] Skipping rebalance for vault ${vaultAddress} — danger response executed or vault not assessed this iteration`);
                        continue;
                    }
                    ...
                    if (shouldRebalance || dryRun || getAllocationDryRun(allocation, vaultEntry)) {
                        ...
                        const currentLedgerInstant = await withTimeout(getCurrentLedgerInstant(kaminoManager.getRpc()), RPC_REQUEST_TIMEOUT_MS, `[allocation-rebalance-loop] get current ledger instant ${vaultAddress}`);
                        // Extract fixed reserves weights for FIXED_WEIGHTS strategy
                        let fixedReservesWeights;
                        ...
                        try {
                            if (typeof vaultEntry !== 'string') {
                                validateConfiguredReserveMembership(vaultAddress, getReserveAddresses(vaultEntry.fixedReserves), kaminoManager.getVaultReserves(vaultState));
                            }
                            ...
                            try {
                                rebalanceIxs = await buildBlacklistEnforcementIxs(kaminoManager, kaminoVault, vaultsReserves, allocationAdmin, dangerDirective.blacklistedReserves, dangerDirective.cooldownReserves);
                            }
                            catch (enforcementError) {
                                logger.error(`[allocation-rebalance-loop] could not build blacklist enforcement for vault ${vaultAddress}: ${enforcementError}`);
                                continue;
                            }
                        ...
                        try {
                            ...
                            if (hasLut) {
                                const lutState = await withTimeout(getLut(c.getRpc(), vaultState.vaultLookupTable), RPC_REQUEST_TIMEOUT_MS, `[allocation-rebalance-loop] get lut ${vaultState.vaultLookupTable}`);
                                luts.push(lutState);
                            }
                            const batchSize = 2;
                            ...
                            await withTimeout(kaminoVault.reloadState(), RPC_REQUEST_TIMEOUT_MS, `[allocation-rebalance-loop] reload vault ${vaultAddress}`);
                            const investLedgerInstant = await withTimeout(getCurrentLedgerInstant(kaminoManager.getRpc()), RPC_REQUEST_TIMEOUT_MS, `[allocation-rebalance-loop] get invest ledger instant ${vaultAddress}`);
                            const investIxs = await withTimeout(kaminoManager.investAllReservesIxs(allocationAdmin, kaminoVault, investLedgerInstant), RPC_REQUEST_TIMEOUT_MS, `[allocation-rebalance-loop] build invest instructions ${vaultAddress}`);
                            await sendInstructionBatches({
                                connectionPool: c,
                                payer: allocationAdmin,
...
//# sourceMappingURL=allocation_loop.js.map