...
import { getAtasWithCreateIxsIfMissing, getCurrentLedgerInstant, lamportsToDecimal, } from '@kamino-finance/klend-sdk';
...
import { getWalletBalances } from './libs/utils/wallet.js';
import { getAllUIEnabledVaults } from './libs/utils/kaminoResources.js';
import { KSwapWrapper } from './services/kSwapWrapper.js';
...
export async function runInvestLoop(cluster, heartbeat) {
    logger.info('✅ running investing loop');
    const { envConfig, connectionPool: c, kaminoManager: initialKaminoManager, refreshKaminoManager, } = await createLoopContext(cluster);
    const kaminoManagerRefreshCoordinator = new KaminoManagerRefreshCoordinator(initialKaminoManager, refreshKaminoManager);
    const investor = await createKeyPairSignerFromBytes(Buffer.from(JSON.parse(readSecret('investor_keypair', 'INVESTOR_SECRET_PATH'))));
    ...
    const baseVaultsPubkeys = envConfig.investVaultKeyOverrides;
    let vaults = [];
    ...
    for (let i = 0;; i += 1) {
        ...
        const tokenMintsWithProgramAndDecimalsMap = new Map();
        const tokensToVaultsCount = new Map(); // for a token mint how many vaults we have with that mint
        ...
        const mintsWithDecimalsAndTokenProgram = Array.from(tokenMintsWithProgramAndDecimalsMap.entries()).map(([mint, { program, decimals }]) => ({
            mint,
            tokenProgram: program,
            decimals,
        }));
        const { createAtaIxs } = await withTimeout(getAtasWithCreateIxsIfMissing(c.getRpc(), investor, mintsWithDecimalsAndTokenProgram.map(({ mint, tokenProgram }) => ({
            mint,
            tokenProgram,
        }))), RPC_REQUEST_TIMEOUT_MS, '[investing-loop] get investor token accounts');
        ...
        const walletBalances = await withTimeout(getWalletBalances(c, mintsWithDecimalsAndTokenProgram, investor), RPC_REQUEST_TIMEOUT_MS, '[investing-loop] get wallet balances');
        const nonSolFundingNeeds = [];
        ...
        if (nonSolFundingNeeds.length > 0) {
            ...
            if (referencePrices) {
                for (const { mint, outputBaseUnitsNeeded } of nonSolFundingNeeds) {
                    ...
                    try {
                        await wrapOrSwap(c, kSwapWrapper, investor, WRAPPED_SOL_MINT, mint, lamports(BigInt(outputBaseUnitsNeeded.toString())), swapConfig, [], 'investing-loop KSwap exact-output swap for crank funds');
        ...
        const vaultsReserves = await loadVaultsReservesInBatches(kaminoManager, vaultsStates, '[investing-loop] load vault reserves', heartbeat);
        const currentLedgerInstant = await withTimeout(getCurrentLedgerInstant(c.getRpc()), RPC_REQUEST_TIMEOUT_MS, '[investing-loop] get current ledger instant');
        heartbeat?.();
        ...
        for (const vault of vaults) {
            ...
            const vaultHoldings = await withTimeout(kaminoManager.getVaultHoldings(vaultState, currentLedgerInstant, vaultsReserves, currentLedgerInstant), RPC_REQUEST_TIMEOUT_MS, `[investing-loop] get vault holdings ${vault.address}`);
            const pendingEvacuations = dangerDetector.getPendingEvacuationReserves(vault.address.toString());
            if (shouldBlockVaultInvestmentForDanger(kaminoManager, vaultState, vaultHoldings.investedInReserves, blacklistedReserves, pendingEvacuations)) {
                logger.warn(`[investing-loop] skipping vault ${vault.address}: it has a pending evacuation or remaining blacklisted exposure`);
                continue;
            }
            // TODO: also consider amount to remain uninvested after it is released
            const theoreticalComputedAllocation = await withTimeout(kaminoManager.getVaultComputedReservesAllocation(vaultState, currentLedgerInstant, vaultsReserves, currentLedgerInstant), RPC_REQUEST_TIMEOUT_MS, `[investing-loop] get computed allocation ${vault.address}`);
            ...
            const allAllocationsZero = Array.from(theoreticalComputedAllocation.targetReservesAllocation.values()).every((allocation) => allocation.eq(0));
            ...
            if (vaultReservesPubkeys.length === 0 || allAllocationsZero) {
                ...
                theoreticalComputedAllocation.targetReservesAllocation.forEach((allocation, reserve) => {
                    ...
                    const reserveAvailableLiquidityLamports = reserveState.getFreelyAvailableLiquidityAmount(currentLedgerInstant, 0);
                    const reserveAvailableLiquidityTokens = reserveAvailableLiquidityLamports.div(new Decimal(10).pow(reserveState.state.liquidity.mintDecimals.toNumber()));
            ...
            if (shouldSkipVault) {
                logger.info(`[investing-loop] skipping vault ${vault.address} as necessary tokens cannot be withdrawn from reserves and there are not enough tokens available to trigger an invest`);
                continue;
            }
            ...
            const configMinSlotsSinceLastInvest = getMinimumSlotsForDurationSeconds(envConfig.minSecondsSinceLastInvest, kaminoManager.recentSlotDurationMs);
            ...
            if (shouldInvestAmountBased && shouldInvestTimeBased) {
                ...
                if (vaultState.vaultLookupTable !== DEFAULT_PUBLIC_KEY) {
                    try {
                        const lut = await withTimeout(getLut(c.getRpc(), vaultState.vaultLookupTable), RPC_REQUEST_TIMEOUT_MS, `[investing-loop] get lut ${vaultState.vaultLookupTable}`);
                        luts.push(lut);
                    }
                    catch (e) {
                ...
                const investIxs = await withTimeout(kaminoManager.investAllReservesIxs(investor, vault, currentLedgerInstant), RPC_REQUEST_TIMEOUT_MS, `[investing-loop] build invest instructions ${vault.address}`);
        ...
        await new Promise((resolve) => setImmediate(resolve));
        // sleep
        const sleepMilliseconds = Math.min(envConfig.loopIntervalMs, kaminoManagerRefreshCoordinator.millisecondsUntilRefresh());
...
async function getVaultsData(c, vaultsPubkeys, ownersPubkeys, kaminoManager) {
    ...
    if (shouldUseOverrides) {
        ...
        if (ownersPubkeys.length > 0) {
            ...
            const vaultStates = (await withTimeout(Promise.all(vaultsPromises), RPC_REQUEST_TIMEOUT_MS, `[investing-loop] fetch vaults for ${ownersPubkeys.length} owners`)).flat();
            for (const vaultState of vaultStates) {
                if (!vaultState) {
                    continue;
                }
        ...
        uniqueVaults = await withTimeout(kaminoManager.getAllVaults(), RPC_REQUEST_TIMEOUT_MS, '[investing-loop] fetch all vaults');
    }
...
async function getVaultsInBatches(kaminoManager, vaultPubkeys) {
    const batches = chunks(vaultPubkeys, VAULT_FETCH_BATCH_SIZE);
    const vaults = [];
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex += 1) {
        const batch = batches[batchIndex];
        const vaultStates = await withTimeout(kaminoManager.getVaults(batch), RPC_REQUEST_TIMEOUT_MS, `[investing-loop] fetch vault batch ${batchIndex + 1}/${batches.length}`);
...
//# sourceMappingURL=investing_loop.js.map