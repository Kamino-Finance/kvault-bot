    ...
    transports: [
        process.env.NODE_ENV !== 'production'
            ? new transports.Console({
                format: format.combine(format.colorize(), format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }), format.printf(({ timestamp, level, message, stack, cause, ...meta }) => {
                    ...
                    while (currCause) {
                        ...
                        const { message: causeMessage, stack: causeStack, ...rest } = currCause;
                        ...
                        if (Object.keys(rest).length > 0) {
                            msg = `${msg}\n${JSON.stringify(rest, (key, value) => typeof value === 'bigint' ? value.toString() : value)}`;
                        }
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
